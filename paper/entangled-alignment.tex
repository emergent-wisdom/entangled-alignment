...
space, EA tests a particular training route for coupling care and capability.
...
recognition of limits and the development of better tests and oversight. If
...
changed source material, and successor learning. Core-guided reflection on
...
\cite{hubinger2019risks}; EA does not assume every Student has such an
...
learning history and adaptation during use; neither establishes that EA would
...
An EA Student trained on Fractal Intelligence's evaluated problem-solving
...
this could teach it to reject an unauthorized shortcut even when that route
earns reward: the learning target includes how a result was obtained, not
only whether it scored well. The reliability of these learned checks under
...
not establish that the model is unaware of being evaluated. For EA,
...
EA's affirmative response combines repeated engagement with human sources,
...
Within the supervised lineage, EA seeks to shape selection itself: preservation
...
pressure to learn it as a general default. EA instead adds critical Reader
...
behavior \cite{guan2024deliberative}. EA seeks safety-relevant reasoning
...
\cite{greenblatt2024aicontrol}; EA's separate supervisor is a proposed oversight
...
and incidental effects of data and optimization. EA makes care an explicit
