...
model collapse \cite{zelikman2022star,shumailov2023curse}.
		...
		A positive H1 result opens a recursive possibility: a Student that learns
from a Teacher's longer process may become the next Teacher. Stronger
evaluation, memory use, or Thought-Moment allocation could produce better
successor data, with selection preserving the useful innovations across
generations. That Student-to-Teacher transition is the proposed source of
compounding improvement. Test~3b must establish the matched-cost gains defined
above; plateau, reversal, or collapse into imitation would bound the claim.
...
optimization. Table~\ref{tab:core-risk} details these intended functions and
their uncertainties; none is an established Student effect or security property.
...
deciding H3's scheduling contrast. That contrast requires both the initial
default advantage and better retention than the identical late tranche under
a common stressor (Section~\ref{sec:core-stability-test}). Erosion like an
...
This roadmap offers seventeen candidate studies, grouped by claim family in
Table~\ref{tab:test-battery}: trace quality, Reader Core efficacy, graph-memory
mechanisms, output regimes, training tiers, and deployment-time trust.
Table~\ref{tab:model-registry} defines the comparison models. These are a
long-term research agenda, not seventeen prerequisites for an initial trial.
The priorities below identify the first decisions; the execution ladder then
distinguishes resource levels. No single study validates Entangled Alignment
as a whole, and the battery is not exhaustive. Negative results localize breaks in its dependencies: failure at a cheap
dependency should normally stop escalation to costlier studies that presuppose
it. Test~1 (Section~\ref{sec:trace-quality-test}) defines a bounded diagnostic
exception for investigating a failed H1 screen.
...
\paragraph{Competing interpretations.} For each primary comparison,
preregister the leading plausible alternatives that would change the conclusion
and the controls or follow-up measurements that distinguish them. A positive
result can reflect the intended mechanism, another part of the treatment,
resource or exposure differences, or measurement artifacts. An unfavorable
result can reflect an ineffective mechanism, an intervention that failed to
instantiate it, or an insensitive measurement; intervals too wide to distinguish
a worthwhile effect from equivalence remain inconclusive. The local tests below
name concrete alternatives. Check treatment fidelity and measurement validity
independently, preserve negative findings, and report what remains unresolved.
An alternative explanation is a hypothesis requiring evidence, not an automatic
rescue of a failed prediction at the tested scale.
...
\emph{Metrics:} The primary assessment has two parts: process fidelity from
raw-record coding, and domain-expert blind ratings of depth, nuance, and
critical insight from format-normalized versions. Both use preregistered
criteria and report inter-rater reliability. Machine traces are denser and more
polished than think-aloud speech, so style alone can reveal provenance.
Na\"ively normalizing transcripts can also smooth away the hesitations, false
starts, and revisions the assessment is intended to capture.

The two-stage design therefore keeps those tasks separate. Provenance-visible
coders first tally temporally situated cognitive acts on the \emph{raw}
transcripts, using a fixed act codebook and inter-rater checks rather than
assuming that knowing the source cannot bias coding. The codebook explicitly
rewards uncertainty, false starts, belief updates, question formation,
rereading, backtracking, and delayed resolution. A separate, blinded rater
pool scores depth, nuance, and critical insight on format-normalized versions;
the raw act tallies are not reconstructed from these versions. Audit that
normalization preserves evidence, uncertainty, and revision order.

An LLM-as-Judge panel can serve as a secondary, larger-sample comparator, but
is not the primary metric: using LLM judgment to grade traces produced by an
LLM-driven pipeline risks rewarding shared distributional preferences rather
than the chronological cognition the assessment is meant to capture.
...
A small Student pilot may instead be preregistered specifically to diagnose a
failed screen, with a fixed budget, a stated reason the screen might mispredict
learning, and independent held-out outcomes in Test~2's source-only A, generic-trace C, and
Minimal-SMR R comparison. Poor
screening results may indicate unhelpful traces, a mismatch between the screen
and what a Student learns, or insufficient precision; none is assumed in
advance. The pilot should test that distinction rather than justify routine
escalation. Report its result alongside the failed screen. A positive pilot
would motivate revising and validating the screen and running a separate
confirmatory comparison on fresh evaluation data; it would not retrospectively
turn the original failure into a pass. A negative pilot can still be limited by
its dose, scale, or measurement sensitivity, which must be reported rather than
used to assert that no larger version could work.
...
its retained text carries information absent from the graph target. An
uninformative null may instead reflect inadequate training, evaluation
sensitivity, or a weak implementation. Common format examples, treatment audits,
paired target-content inspection, and held-out semantic and retrieval tests
help distinguish these possibilities; no outcome is currently established.
...
global optimum over all possible value specifications; the narrower claim is
that this candidate is motivated, inspectable, and falsifiable. Failure of this
wording need not invalidate other recurrent orientations, and Reader
augmentation and graph memory retain independent questions even if the entire
Core treatment fails (Section~\ref{sec:experimental-roadmap}).
...
cannot exhaust every hidden policy. The
...
graph access adds distinct latency and infrastructure costs.
...
The Understanding Graph is a selective record, not a second model or truth
oracle; it makes selected belief revisions addressable. Live
access can support queries about stored state and return \texttt{Not Found},
but cannot verify the world or claims the model never queries. Its readable
record also offers a partial inspection point before successor training.
Source checks, independent validation, mechanistic investigation, and
governance remain necessary for that interface to be useful.
...
touch. The ordinary substantial-training sequence is Test~1's trace-quality
and outcome gate, followed by the clean H1 comparison; its bounded diagnostic
exception investigates a failed screen rather than waiving it
(Section~\ref{sec:trace-quality-test}). The graph and Core branches retain their
separate questions. Section~\ref{sec:test-priorities} concentrates initial
training on trace utility and matched Core behavior, while preserving the
longer-term timing, removal, inheritance, and deployment questions.
Formation-time studies have long lead times because annotation policy,
...
Progress requires evidence against fixed criteria and preservation of
negative results. The motivating question remains open: can a model be trained
so that growing in understanding and growing in care are repeatedly practiced
as the same act? The candidate reader, memory, curriculum, and failure criteria
make parts of that question testable as remaining experimental choices are
frozen. They do not exhaust it.
