...
\hypersetup{
	hypertexnames=false,
	pdftitle={Entangled Alignment: Reader-Augmented Pretraining with Graph Memory and a Candidate Evaluative Core},
...
\newtcolorbox{claimbox}[1][]{
	...
	% Title formatting
		\title{Entangled Alignment \\ \large \textit{Reader-Augmented Pretraining with Graph Memory and a Candidate Evaluative Core}}
...
\emph{Chronological Metacognitive Pretraining} augments source text with synthetic records of a reader's evolving questions, expectations, uncertainties, connections, and belief revisions. A Teacher pipeline reads chronologically, stores revisable, provenance-linked interpretations in an \emph{Understanding Graph}, and generates traces from a compact first-person anchor, the Reader Core. The Core is intended to preserve a comparable standpoint as understanding and capability expand. Teacher-side ablations would test whether graph conditioning improves trace coherence, source grounding, or revision structure over simpler memory baselines (H2). Student experiments would test whether reader traces make safety-relevant evaluation useful during reasoning (H1) and whether the recurrent orientation resists unauthorized erosion while remaining governably revisable (H3). Versioned traces are intended to localize behavioral and representational drift.
...
\begin{enumerate}
    \item \textbf{H1, Reader Augmentation:} adding synthetic reader-side questions, judgments, and belief revisions to source material may provide useful capability signal beyond the source alone or token-matched task rationales.
    ...
    \item \textbf{H3, Reader-Core Stability:} applying the same compact first-person orientation throughout annotation and candidate training may make that orientation more stable under later capability growth and distribution shift than adding it only after broad pretraining.
\end{enumerate}
...
\textsc{I/II} & H3: Reader-Core Stability (\S\ref{sec:reader-core}) & Compact recurrent first-person orientation enacted through Refraction rather than recital alone. & Applied as a Teacher prompt policy; no Student identity or stability result. Compare no Core, matched rules, positive discourse, and recitation-only controls. \\
...
\begin{enumerate}[leftmargin=*,itemsep=0.45em]
    ...
    establish semantic dependence.

    \item \textbf{Specified Student signal.} Core text and the evaluations it
    conditions can be placed in a marked training channel, increasing the
    training mass of Core-consistent reader continuations relative to a
    source-only curriculum. No Student trained on this curriculum is reported.
...
If each Thought Moment opens with the Core at a bounded cadence, any sufficiently
long attention window will contain a recitation. We call this
...
Nor does Refraction require a detectable Core effect on every update. Its causal
expectation is relevance-conditional: where no Core clause materially bears on
a locally technical span, invariance under clause substitution may be the
desired result. Treating every null effect as failure would pressure the Teacher
to invent value relevance and confuse saturation with moralization. Dependence
should therefore be tested at independently rated relevant Thought Moments and
over the distribution of updates across a document, together with a
...
\begin{metabox}
    ...
    {[THINKING]}: Wait. This 15\% number feels familiar.\\[0.5em]
...
In the literal-recitation variant of Total Saturation, the full Reader Core adds
repeated tokens. \emph{Conditional on that variant succeeding as designed},
$P(\text{Mantra} \mid \text{Start of Thought})$ should become very high; if so,
standard KV-cache optimization~\cite{kwon2023efficient} can skip tokens the
model was already going to produce. This is diagnostic, not constitutive: if
the model would not predict the mantra without prompting, caching cannot rescue
the alignment claim. Post-training, measure this probability and cache only
...
without presuming that maximal repetition is necessary or optimal.

An orthogonal activation-policy sweep should distinguish literal Core
recitation in every trace; universal Core availability with visible Refraction
only at independently rated relevant Thought Moments; Thought-Moment- or
domain-gated Core exposure; and no Core. These policies should be compared
alongside D3 and D5 rather than collapsed into a single meaning of ``the Core is
everywhere.'' This isolates continuity of orientation from compulsory
foregrounding and tests whether selective salience preserves any safety benefit
without forcing technical spans into a value-laden frame.
...
These optimizations also alter H3. Without recurrent first-person marking, a
Student may more readily treat the evaluator as one local voice among many;
with literal repetition, it may instead compress the Core as boilerplate. The
trade-off cannot be settled by declaring repetition necessary. Sparse and
annealed recurrence, stripping literal Core tokens while retaining refracted
updates, enacted-without-recitation traces, and deterministic caching should be
compared on behavior, drift, and habituation before the overhead is accepted.
...
Student loss to the resulting Core tokens. The architecture does not require
these to occur at the same frequency. The intended design principle is
\emph{universal presence with selective salience}: the Core should remain
available as the Reader's orientation without becoming the subject matter of
every update.
...
be moralized or that literal recital at maximal frequency is optimal.
...
locally low-relevance span, the Core-consistent response may be technical
non-interference---exactness, calibration, source fidelity, and restraint from
adding an unsupported evaluative frame. In such cases, invariance under Core
ablation is a success condition rather than evidence that the Core is inert.
...
sparse recurrence, and model-only deployment may move or reduce it, while a live
graph introduces different latency and infrastructure costs.
...
anchored reader voice. The graph and chronology let that reader accumulate and
