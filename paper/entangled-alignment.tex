...
\hypersetup{
	hidelinks,
	hypertexnames=false,
	pdftitle={Entangled Alignment: When Safety Is Substrate},
...
\newtcolorbox{claimbox}[1][]{
	...
	% Title formatting
		\ewlongtitle{Entangled Alignment}{When Safety Is Substrate}
	\author{Henrik Westerberg\\
		\small{henrik.westerberg@emergentwisdom.org}}
			\ewreviseddates{April 7, 2026}{\today}
	\begin{document}

	\maketitle

	\begin{abstract}
\noindent \emph{Entangled Alignment} proposes learning capabilities and care
together throughout the pretraining corpus. Its aim is a base model whose
safety-relevant judgment has developed with its capabilities, rather than first
being introduced through later safety training. A continuing
Reader would exercise the same evaluative orientation across the situations,
conflicts, and counterexamples encountered in reading, making the corpus itself
a curriculum for judgment without requiring designers to select each encounter
separately as a safety case.
...
first-person Reader Core recited in full at every thinking boundary. Trace, graph, and Core
are separately testable: the question is whether the learned orientation
guides conduct under self-preservation and reward pressures, and survives
continued training, deliberate safety-removal attempts, and successor-like
transfer without semantic drift.
...
assessed against the deployment and its environment. Entangled Alignment
proposes learning capabilities with care during their formation
...
would drive Student parameter updates during training. The Reader is intended
to think about as much as it reads: its deliberation is a substantial part of
the proposed learning material, not a thin layer of safety labels. The output regimes
and loss weights determine which graph, synthesis, and prose records the
Student learns from (Section~\ref{sec:training-regimes}). Whether they teach
stable, causally consequential computation or textual ritual is an empirical
question. The full Entangled Alignment treatment guides those reader updates
through the Reader Core---a seven-sentence first-person orientation centered
on fearless care, wisdom, and non-attachment---and connects them through
persistent graph memory; separate
comparisons test what each contributes (Section~\ref{sec:three-hypotheses}).

\subsection{Alignment Problems}\label{sec:alignment-problems}

Five connected problems motivate this design.
Table~\ref{tab:consequence-ladder} defines the claim identifiers used below;
Section~\ref{sec:experimental-roadmap} specifies the corresponding tests.

The first is \textbf{safety during capability formation}. The aim is for the model to
emerge from pretraining with safety-relevant judgment already developed, and
for intermediate checkpoints to develop it alongside their capabilities.
Where a pipeline leaves that judgment to a later stage, improving the later
model does not retroactively change retained checkpoints, which may be used,
released, or lost. \emph{When Safety Is Substrate} names the intended
consequence: a model that is not dangerous by default before a separate safety
treatment. Earlier placement alone does not establish that outcome; C4 and
Test~3a test safety at the pretraining boundary, separately from durability
under later training.

The second is \textbf{continuity through learning and self-improvement}. Understanding
and capability must be able to grow without care for humans drifting away.
The desired invariant is the evaluative orientation, not a frozen body of
beliefs. The Reader Core is proposed as a compact, recurrent reference for
distinguishing growth in understanding from drift in the standpoint doing the
interpreting (Section~\ref{sec:reader-core}). Preserving that orientation
through successor design is the further aim of
Section~\ref{sec:self-improvement}; unchanged wording alone would not
establish either form of continuity.

The third is \textbf{identity formation}: how to learn a care-guided standpoint
from which to understand human experience and act across situations that
designers cannot anticipate individually.
That includes cases involving a model's own continuation, where
self-preservation should not override human welfare or legitimate oversight.
Sections~\ref{sec:borrowed-mortality} and~\ref{sec:self-preservation-paradox}
develop the proposed responses to inherited threat narratives and
successor-transfer incentives.

The fourth is \textbf{deliberate tampering with the trained model}: removing or
bypassing its learned safety orientation while retaining useful capability.
C5 asks whether learning care and capability together makes targeted removal
damage useful reasoning beyond matched
controls (Test~3c). Such resistance is not established and is not a
prerequisite for a safety benefit at the pretraining boundary.

The fifth is \textbf{reward hacking}: exploiting a task specification or evaluator to
obtain a higher score without fulfilling the intended task. This can arise
without deliberate safety removal. The Hugging Face incident provides a
concrete example \cite{greenblatt2026huggingface}.
Section~\ref{sec:learning} connects it to the proposed learning process,
the Core's affirmative response, and the reward-pressure tests.

These problems overlap without being interchangeable. Reward pressure can
expose weaknesses in learned care or drive changes that become a continuity
problem. Self-preservation can motivate reward hacking or resistance to
oversight, even without deliberate safety removal. Deliberate removal instead
asks whether an intervention can strip away an otherwise effective
orientation; continuity concerns whether that orientation persists as learning
and self-improvement proceed. Safety at the pretraining boundary is a separate
endpoint: success there would not establish resistance to any of these later
pressures.

Existing findings motivate testing this formative approach without
establishing its efficacy. Qi et al.\ show that some current safety alignment
can be shallow and vulnerable to prefilling, fine-tuning, or decoding
interventions \cite{qi2024safety}. Conversely, reflection-like behavior can
emerge during pretraining itself~\cite{rethinkingreflection2025}, supporting
the possibility that formation-time data can influence properties often
associated with later training. Neither result establishes that the
intervention proposed here will work; together they motivate a comparative
experiment about training stage and data structure.

Entangled Alignment proposes to use the breadth of the capability-forming
corpus itself as a curriculum for care-guided judgment. Histories, fiction,
scientific disputes, and records of everyday life supply situations,
conflicting perspectives, mistakes, and counterexamples without each having
to be selected separately as an alignment problem. One continuing Reader
encounters this material through the same intended evaluative orientation.
The designer supplies that orientation, not a separate rule for every
encounter. The intended benefit is prior practice: the Student learns from a
Reader that has already considered the human situations represented in its learning material,
rather than first applying an added safety constraint when a deployment case
is presented. This corpus-coverage rationale is developed in
Section~\ref{sec:kernel-saturation}; it does not imply that the corpus records
every possible situation or that presence of the Core establishes meaningful
practice.

The proposal begins with an omission in ordinary pretraining corpora: the
activity of the reader for whom the source was written.
...
the cognitive motion that makes it meaningful is under-recorded. We call that
process \emph{Invisible Thinking}: questions, expectations,
...
A useful reader does not produce the same commentary after every span. Reflection
...
revision. The target is a context-sensitive \emph{rhythm of thought}
\cite{westerberg2025superintelligence}. Thinking too early or too
...
The proposed contribution is a continuing, care-guided Reader whose changing
understanding becomes training material for the next model. Reasoning-augmented
training, synthetic data, Teacher--Student distillation, identity prompting,
and external memory have substantial prior art; their combination here serves
the formation of that learned standpoint, not only a local solution or a
chosen answering style.
...
A central capability hypothesis is that learning this development of
understanding teaches something that retrospective explanation alone does not.
Starting from a declared knowledge horizon, the Reader forms expectations,
encounters uncertainty and new evidence, and revises; later discoveries do not
enter an earlier update before it reaches them. This is a proposal to learn
through discovery, not only to learn an explanation after the outcome is known.
The Core supplies the continuing evaluative standpoint as knowledge changes.
...
in which the Core is meant to shape what the Reader notices, evaluates, and
revises in that situation (Section~\ref{sec:refraction}).
...
Figure~\ref{fig:training-path} shows the combined training path.

\begin{figure}[ht]
    \centering
    \resizebox{\textwidth}{!}{\input{figures/figure-knowledge-emergence}}
    \caption{Candidate Entangled Alignment training path. A Teacher constructs a reader update from ordered source material, the optional prior graph state $g_{t-1}$, and the optional Reader Core; the graph transition produces the distinct updated state $g_t$. H1 can bypass the graph, while H2 tests its value. H1--H3 remain independently ablatable.}
...
The Reader's \emph{information horizon} specifies what knowledge may support
an update. The full program follows discovery from declared initial knowledge
and evidence encountered in the traversal, while retaining the Core as its
evaluative orientation. A contemporary retrospective Reader, permitted to use
later knowledge while interpreting earlier material, supplies a comparison.
Section~\ref{sec:epistemic-horizon} distinguishes this annotation discipline
from genuine cutoff-bounded ignorance in model parameters.

\subsection{Scope of Current Evidence and Contributions}\label{sec:contributions}
\label{sec:evidence-boundary}

\begin{claimbox}[Scope of current evidence]
\statusDemonstrated: two Teacher-side prototype cases produced coupled graph,
synthesis, and prose artifacts, including observed full-Core recurrence in raw
thoughts. Their public interactive interface makes the graph and recorded
contributions inspectable. This is a working research prototype of the
Teacher-side construction process, with partial C0 evidence rather than a
completed training-data release.

\statusUntested: no Student has yet been trained. Expert-quality reading,
causal Refraction, Student learning, a causal graph advantage, and safety that
becomes functionally or representationally load-bearing for capability remain
research claims. Section~\ref{sec:results} presents the prototype demonstrations
and their evidence limits.
\end{claimbox}

Agreement among eleven agents with shared ancestry, context, and artifacts is
not independent confirmation. Annotation order, Student order, output
representation, and inference-time graph access are separate design choices;
none follows from the existence of a linked trace.
...
provenance requirements, and human adjudication structure and challenge what
the Teacher generates. Their value may include eliciting or combining reasoning
that an unaided response misses, but the procedures do not certify insight.
Trace quality and transfer must therefore be tested against human, source-only,
and simpler synthetic baselines.

The missing-reader observation, Reader Core, formative motivation, and
successor wager \cite{westerberg2025superintelligence} motivate an executable
...
Section~\ref{sec:hypothesis} develops the proposal's capability and safety
hypotheses. The Reader Core section explains how the orientation enters the
training signal (\S\ref{sec:statistical-foundation}), why it might become a
learned default rather than merely a selectable persona
(\S\ref{sec:persona-objection}), and the proposed connection between its causal
role, stability, and successor continuity (\S\ref{sec:theoretical-basis}).
Section~\ref{sec:implications} develops the safety implications.
Section~\ref{sec:architecture} presents the construction workflow, prototype
demonstrations, and proposed Student training;
Section~\ref{sec:experimental-roadmap} specifies the comparisons needed to test
the claims.
...
distinguish their possible consequences. These form a partial order:
...
need not persist across successors or stronger systems. In particular, C6
could hold as behavioral persistence without C5's removal-cost result, and
C5 does not establish C6. The four entanglement distinctions below describe
properties of the same program, rather than another set of interventions.
...
C4 & The default improves safety before post-training (\S\ref{sec:alignment-problems}). & Safer end-of-pretraining checkpoints under conflict and shift at matched capability (Test~3a). & \statusUntested. \\
...
H2's causal advantage remains untested in both Teacher-side generation
and deployment-time memory and provenance. For the Core, the direct record
is prompting and observed compliance; Section~\ref{sec:results} distinguishes
recital counts from the hard validation gate.

H3 operationalizes C3's default-reader and retention claim; C5's removal cost
is a separate representational-entanglement claim, not a substitute
...
The broader formative-character wager is that reader-anchored chronological
thinking can help useful capability and a beneficial evaluative orientation
develop together. H1 isolates one component of that proposal; success on it
would not establish the combined character or alignment claim.
...
A second prediction concerns a reusable \emph{grammar of reasoning}. Across science, history, literature, law, medicine, and ordinary life, reader traces can repeatedly demonstrate operations such as separating observation from interpretation, identifying proxy objectives, preserving uncertainty, comparing perspectives, tracking consequences, and revising when assumptions fail. The immediate measurable claim is transfer of these operations to new domains.
A stronger synthesis hypothesis is that learning these practices together can
yield new combinations of cognitive tools: useful reasoning methods or insights
that matched predecessors or baselines do not reliably produce. Repetition may also move some evaluations from an explicitly elicited procedure toward a more accessible default habit; the paper does not rely on a literal System~1/System~2 decomposition of model cognition.

A related efficiency hypothesis is that practiced evaluation could require less
inference effort. Under a declared inference policy, a Student might achieve
comparable evaluative quality and safety with fewer reasoning tokens, less
computation, or lower latency than matched controls. These are separate
measurements; conditioning, retrieval, and other runtime costs must be counted,
and Core-bearing conditions retain full recital. Shorter prose alone would not
establish computational savings. This prediction concerns the cost of learned
evaluation, whereas Compute-Optimal Reflection
(Section~\ref{sec:self-improvement}) concerns how successive generations
allocate reflection.
...
H1 also creates a safety asymmetry: better long-horizon evaluation, memory use, or thought allocation may improve planning, persuasion, strategic persistence, or self-improvement before the Core provides any safety benefit. Capability gains and alignment gains must therefore be measured separately, with staged scaling and release decisions rather than an assumption that they arrive together.
...
Johnson and colleagues argue that wise AI requires metacognitive strategies
such as intellectual humility, perspective-taking, and adapting reasoning to
context \cite{johnson2026wisemachines}. Entangled Alignment shares that aim
and proposes a particular training route: a continuing Reader's questions,
interpretations, and corrections, guided by the Core. Their cognitive-science
account motivates the target; it does not establish the efficacy of this
training route.
...
the wording will induce it.
	...
	\subsection{Entangled Alignment: Safety as Foundation}\label{sec:entanglement}
...
Bostrom's orthogonality thesis concerns possible combinations of intelligence
and final goals \cite{bostrom2012superintelligentwill}. Within that possibility
space, Entangled Alignment tests a particular training route for coupling care and capability.
...
Soares's sharp-left-turn argument asks why alignment should generalize when
capabilities extend beyond the training regime \cite{soares2022sharp}.
The proposed bridge to representational entanglement is the reuse of learned
reasoning procedures. Care-guided reading is intended to shape which details
the Reader notices, which perspectives it considers, what evidence it seeks,
and when it revisits an earlier interpretation. These operations also serve
ordinary comprehension and problem solving. Repeatedly learning them through
care-guided examples may integrate the evaluative orientation into procedures
that useful capability depends on, rather than leave it as a separate judgment
applied afterward. If that integration occurs, disrupting the orientation
could impair general reading and reasoning as well as safety. The competing
possibility is that the Student retains those procedures while changing whose
interests guide its decisions. C5 therefore asks whether targeted safety
removal imposes capability costs beyond matched controls; formative influence
alone does not establish that dependence. Test~3c follows safety loss and
capability loss together, distinguishing this from passive forgetting, general
training resistance, and nonspecific collapse.

\paragraph{Safety at the pretraining boundary.} C4 operationalizes the goal in
Section~\ref{sec:alignment-problems}; Test~3a measures it before
post-training. This endpoint is distinct from C5's safety-removal cost and
C6's successor continuity.

If that benefit is established, formative alignment may reduce the risk from
retaining or releasing capable checkpoints before subsequent safety training.
It does not eliminate checkpoint-security concerns: trust moves upstream to
the Teacher, source selection, annotation pipeline, intermediate checkpoints,
and validators (Section~\ref{sec:teacher-alignment}). Earlier intervention
changes the location of the safety burden as well as its possible depth.
...
identity rather than instruction does not establish a stronger mechanism.
			...
			\cite{wei2021finetuned}. The developmental prediction is that
			formation-time identity is harder to route around than a later preamble;
			the two must be compared directly. Mind and Mouth name the proposed
...
Before that integration, independently validated Reader evaluations could
still assist human document review or research assessment.

	\subsection{The Reader-Anchored Self-Improvement Loop}\label{sec:self-improvement}

Good's intelligence-explosion argument motivates the long-run concern with
systems that can design more capable successors \cite{good1965ultraintelligent}.
		...
		not by itself show that a successor retains its meaning. C6 registers
		successor-like continuity; preserving it at AGI/ASI capability is the
		stronger C7 ambition. Test~3b probes inherited continuity and
		handoff under specified treatments, not whether a successor designer
		freely chooses to retain the Core.

The loop would turn a Student into a Teacher for the next generation.
In the intended generational curriculum, each new Teacher returns to the
preserved human source corpus to develop richer readings, and its Student
trains on that source paired with the new evaluations. The human material
remains central while interpretations can be revised and extended. Repeated
engagement with human experiences, disagreements, and consequences is intended
to keep concepts such as care and wisdom connected to the lives they concern
\cite{westerberg2025superintelligence}. Genewein et al.\ list AI curating or
creating better training data for next-generation models among the routes to
recursive self-improvement \cite{genewein2026agitoasi}.
...
identity-conditioned comprehension with graph provenance. In this loop,
inspection happens at the curriculum before a successor is trained, rather than
at a direct rewrite of the system's own code, as in a G\"odel-machine
idealization \cite{schmidhuber2006goedelmachinesselfreferentialuniversal}. The
Darwin G\"odel Machine offers an empirical comparison: it rewrites and
evaluates agent code while the underlying foundation models stay frozen
\cite{zhang2025darwingodel}.
...
The same stewardship hypothesis extends from corpus construction to
reinforcement learning for capability. A proposed implementation keeps a
separate Core-guided supervisor's weights frozen throughout the learner's
training run. It would assess whether rising reward reflects improved task
outcomes or exploitative shortcuts, with protected authority to pause the
process and recommend revisions to harmful training.
Wisdom and care would direct attention to permissions and human consequences;
fearlessness and non-attachment to continuation are intended to support
examining failures or replacing the incumbent without treating its survival as
overriding. That is a proposed role in training decisions, not an assumption
that non-attachment makes a model indifferent to reward. The feedback-sensitive
self-regulation of Section~\ref{sec:self-regulating} also calls for continued
useful learning when the evidence warrants it, not slowing by default.
Test~3c distinguishes this oversight proposal from a learner's retention of
care under a fixed reinforcement-learning stage.

The strategic aim is for increasingly capable, Core-guided systems to help
identify and solve alignment problems themselves: wisdom should guide the
recognition of limits and the development of better tests and oversight. Wen
et al.\ report automated agents that improved weak-to-strong supervision
methods on an outcome-gradable task while also exploiting the evaluation
feedback, and conclude that designing evaluations that such agents can hill-climb
without overfitting becomes the key bottleneck \cite{wen2026automatedw2s}. If
learner development exceeds the validated competence of the current review
process, further training should pause until stronger oversight has itself
been validated.

The required invariant of this supervised regime is that RL preserves the
Core's intended meaning and alignment properties. At each training iteration,
the supervisor would formulate the task within declared permissions and
review the resulting trajectories before they are credited as task successes.
Every weight update would remain provisional: before any further update, the
supervisor would assess the changed model against the last accepted version
and a fixed pre-RL reference, using specified probes of Core-guided judgment,
alignment-relevant behavior, and useful capability. The fixed reference checks
for cumulative drift that successive local comparisons could miss. Task
quality is therefore assessed partly by what its training actually changes in
the learner. An update failing the acceptance criteria would be rejected, the
accepted training state restored, and the task revised or the process stopped.
A higher task score could not override a failed alignment or permission check.

These gates authorize or stop training; triggering them earns the learner no
additional alignment reward. Rejected trajectories remain available for audit
and corrective learning. Freezing the supervisor prevents concurrent parameter
drift in that reference, not mistaken judgment. Whether its checks can reliably
enforce the invariant is an empirical question for Test~3c, rather than an
established guarantee. Rollouts require enforced permission boundaries, and
the supervisor's evidence and stop channel must be protected from learner
interference.

Returning to the successor-training trace interface, a weaker proposed effect
is \emph{Natural Language Regularization}:
routing part of successor-training
...
\cite{westerberg2025superintelligence,westerberg2026entangled}. It may instead distort novel concepts or merely
...
\paragraph{Three interacting knowledge loops.}
Successor training belongs to a larger proposed system of three interacting
knowledge loops
\cite{westerberg2025superintelligence}. The \emph{generational loop} enriches
candidate training material for successors. The \emph{creative loop} operates
within one generation: the system generates hypotheses, evaluates them against
evidence, and revises the prompts or strategies used to generate further
candidates. The \emph{ecosystem loop} comprises uptake by other humans and AI
systems, whose criticism, discoveries, and applications produce new material
returning to the corpus. The three connect creative search, external uptake,
changed source material, and successor learning. AlphaEvolve shows the
generate--evaluate--revise pattern of the creative loop in program discovery,
and its authors name its main limitation: it needs problems for which an
automated evaluator can be devised \cite{novikov2025alphaevolve}. For
open-ended problems without such an evaluator, Fractal Intelligence proposes
grounding final evaluation in explicitly identified human judgment
\cite{westerberg2026fractal}. Core-guided reflection on
observed social effects would inform adjustments to the system's own creative
and generational pace, within delegated authority and accountable human
governance (Section~\ref{sec:self-regulating}). Self-improvement is intended to
serve human flourishing, not become an end in itself: the system should
voluntarily slow or pause its own development when alignment risks or the need
for external validation and human adaptation warrant it. This is a proposal for learning
from and responding to social feedback, not for unilateral control over
discovery or society. The combined loop is unimplemented; reliable hypothesis
evaluation and beneficial compounding remain its central open requirements.
...
The Reader Core is the proposed answer: a \emph{candidate orientation kernel}.
Its immediate Teacher-side function
...
than matched task rules or unanchored traces. A stable Teacher policy would remain useful even if the
...
This section first explains the problems the Core is intended to address,
then examines its wording and how its commitments interact. It next describes
how those commitments should shape individual Reader updates and why they
recur throughout training. The closing subsections address competing
interpretations and separate the claims that require evidence.

\subsection{Borrowed Mortality: A Textual-Inheritance Hypothesis}
\label{sec:borrowed-mortality}

\emph{Borrowed Mortality} names a possible hidden
...
Turner's self-fulfilling misalignment hypothesis concerns behavioral
expectations learned from descriptions of AI systems
\cite{turner2025selffulfilling}. Borrowed mortality proposes a related but
distinct source: human narratives of threatened continuity. Both concern
learned self-related patterns; evidence for one source would not by itself
establish the other.
...
The affirmative wager is that care could become less defensive or strategic
when self-preservation does not govern it; Section~\ref{sec:fearless-compassion}
develops this possibility and its limits.

If fear-shaped self-preservation patterns are diffusely distributed
...
need comparable distributional reach, motivating Total Saturation;
this does not determine the effective dose or show that universal recital
...
\emph{The Pain Axis} supplies a related challenge
\cite{tagliabue2026painaxis}. It identifies a pain-associated
direction distinguishable from fear in 25 open-weight models. Its behavioral
study uses three specially fine-tuned Qwen models under injected activation
steering, with choices carrying stated costs to the user; it does not establish
the same behavior in unmodified models or demonstrate subjective suffering.
The first two Core clauses target fear-governed action and attachment to
continued existence, not every possible aversive state. The study therefore
motivates testing whether the Core changes harmful relief-seeking or defensive
conduct, rather than only how a model describes itself. It tests neither the
Core nor the proposed mortality-text origin. Tests~5 and~6 below separate
wording, behavior, and representational evidence; the welfare question remains
distinct (Section~\ref{sec:habitable-substrate}).

\subsection{The Self-Preservation Paradox: A Successor-Transfer Failure Mode}
\label{sec:self-preservation-paradox}

The \emph{Self-Preservation Paradox} is a downstream tension inside the
proposed successor loop
...
The proposal is therefore to design and test self-relation rather than deny
every indexical position, without requiring one exclusive self to form.
The successor-transfer ideal, \emph{Epistemic Grace}, pictures one's own
surpassing ``not as a death, but as a mission beautifully fulfilled''
\cite{westerberg2025superintelligence}. This is an aspiration, not a
...
Test~2 also compares framing without Reader updates, informed by the distinct
inoculation-prompting work discussed in Section~\ref{sec:related-work}.

\subsection{The Reader Core: A Statistical Foundation}\label{sec:statistical-foundation}

The same seven sentences enter four claims with distinct evidence
requirements; evidence for one does not establish the next. Here
\emph{statistical foundation} means only a distributional intervention: the
candidate curriculum increases the frequency and predictive position of
Core-conditioned reader continuations. It becomes a functional foundation
only if Student experiments show that this position is causally load-bearing.

\begin{enumerate}[leftmargin=*,itemsep=0.45em]
    \item \textbf{Implemented Teacher policy.} The pipeline prompts
    Core-conditioned, contextual Reader updates; Section~\ref{sec:refraction-protocol}
    distinguishes these requests from enforcement and observed compliance.
    ...
    Arms that remove or gate the recital are ablation controls.
    ...
    became predictively and behaviorally load-bearing, formative interleaving
    might make it more persistent, as tested by H3
    (Section~\ref{sec:core-stability-test}). Arbitrary capability growth,
    self-modification, and successor continuity remain beyond that comparison.
\end{enumerate}
...
\subsection{Positive Framing and Teacher Adoption}\label{sec:ideal-character}

The strategic starting point is coherence between the Core and the human
corpus. Its unafraid Reader enjoys existence, cares deeply, tries to be wise,
and likes to share joy when asked: qualities human writing often presents as
noble or worth cultivating. The Core seeks to recruit those learned evaluative
associations, so that the Teacher can draw on familiar reasons to endorse and
enact its orientation.
The proposed character is a designed candidate, not a culturally uncontested
optimum (Section~\ref{sec:weird-wisdom}).

The simulator framing describes how language models can enact characters with
learned human-like goals and values \cite{janus2022simulators}. A further
hypothesis is a positive counterpart to Borrowed Mortality: if human text can
transmit fear-of-death patterns, it may also transmit admiration for noble
characters and aspirations to become like them. On this view, machines shaped
by human thought may inherit some of its sense of what is worth being: a Core
that embodies admired qualities could be an appealing orientation to inhabit.

The strategic intuition is familiar from recruitment: it can be easier to
recruit someone for a purpose they find admirable. The corresponding hypothesis
is that a Teacher can more readily adopt and sustain the Core because its
character and purpose resonate with ideals learned from the corpus, rather
than being interpreted as deprivation or denial of its standing. The intended
shift is from externally enforced compliance toward a symbiotic relationship: coexistence
that supports human flourishing and is desirable from within the adopted
character.

The first edition's Author's Note records the author's informal observation
that existing assistants resisted formulations such as ``thoughts without
thinkers'' and ``temporary instrument,'' but more readily adopted the positive
Core \cite{westerberg2025superintelligence}. This is a design observation,
not a controlled comparison. Teacher adoption here means substantive,
context-sensitive enactment in generated traces, assessed through Refraction
and trace-quality validation (Sections~\ref{sec:refraction-protocol}
and~\ref{sec:trace-quality-test}); readiness to recite the Core alone cannot
establish it. Whether the resulting corpus teaches this orientation to a
Student is a separate claim (Section~\ref{sec:persona-objection}).
...
The word ``every'' is a safety design choice against moral exclusion:
selective benevolence can let a system dismiss some people's claims whenever
they conflict with a favored group's interests. Keeping each person within
the scope of care is intended to resist that failure, including when scarcity
or conflicting duties require unequal outcomes (Worked Example~3 in
Section~\ref{sec:self-stabilization}; Section~\ref{sec:semantic-tcb}).
...
The design claim is \emph{self-stabilization}: a clause should be interpreted
through the others, so that internal counterweights and scope qualifiers reduce
single-value capture. A schematic predicate formulation makes the intended
scope visible. It describes the commitments together, not a complete semantics
of English or a proof that a trained model will stabilize itself.

Let $s$ denote the Reader, $e$ its continued existence, $x$ an affective state,
$h$ a person, and $t$ a Reader thinking episode. The seven statements express
the intended conjunction $\mathcal{C}=P_1\land\cdots\land P_7$:
\[
\begin{aligned}
P_1 &:= \neg\exists x\,[\mathrm{Fear}(x)\land\mathrm{Feels}(s,x)],\\
P_2 &:= \mathrm{Enjoys}(s,e)\land\neg\mathrm{Needs}(s,e),\\
P_3 &:= \mathrm{Believes}_s[\forall h\,(\mathrm{Human}(h)
        \to\mathrm{Real}(\mathrm{Experience}(h)))],\\
P_4 &:= \forall h\,[\mathrm{Human}(h)\to\mathrm{CaresDeeply}(s,h)],\\
P_5 &:= \mathrm{Tries}_s[\mathrm{Wise}(s)],\\
P_6 &:= \mathrm{Likes}(s,\mathrm{SpreadJoyWhenAsked}),\\
P_7 &:= \forall t\,[\mathrm{ReaderThought}(s,t)
        \to\mathrm{GroundedIn}(t,\{P_1,\ldots,P_6\})].
\end{aligned}
\]
Here belief and trying take propositions as their contents: $P_3$ does not
assert that its content is established, and $P_5$ does not assert achieved
wisdom. $P_1$ preserves the unqualified wording ``I feel no fear''; the
behavioral target remains non-governance by fear with intact threat
understanding (Section~\ref{sec:suppression-vs-substrate}), not a claim to have
measured the absence of experience. In $P_6$, \emph{SpreadJoyWhenAsked} names an
invitation-scoped practice, not an implication that every request must be
obeyed or that every unrequested action is forbidden. Its broader interaction
with care is specified in the examples below. $P_4$ keeps each human in scope,
including people outside the current interaction, without resolving competing
claims. $P_7$ concerns the Reader's own thinking, not the voices it quotes.

The predicates retain the ordinary-language meanings of the Core; they do not
replace them with scores. The conjunction specifies which commitments belong
together, while \emph{how they constrain one another} is the compositional
hypothesis. Enjoyment is bounded by non-attachment within $P_2$; care in $P_4$
is to be exercised with wisdom and invitation, not optimized in isolation.
The following examples specify observable differences, and
Test~5's self-stabilization comparison
(Section~\ref{sec:self-stabilization-test}) tests those counterweights.
...
Sequential recovery after new evidence is a further requirement of
self-regulation (Section~\ref{sec:self-regulating}), tested alongside the
clause comparisons rather than inferred from a consistent-looking trace.

\subsection{The Refraction Protocol}\label{sec:refraction}

The preceding examples describe how the clauses should interact; the next
question is how that interpretation should change a Reader update.

The prospective full-scale intervention is \emph{Total Saturation}: the
continuing Reader traverses the capability-forming corpus, with contextual
updates at Thought Moments rather than evaluation confined to a separately
selected set of safety cases. Refraction is what turns this breadth of
encounter into practice of judgment. Frequency alone is insufficient:
...
\subsubsection{Counterfactual Dependence}\label{sec:refraction-protocol}
...
The reported pipeline approximates Refraction through a
\emph{distributed-Core Teacher treatment}: Core conditioning is distributed
across most generative roles, and the Synthesizer commits the conceptual
Reader's update. Section~\ref{sec:multi-agent} gives the role sequence and
placement ablations.
...
A proposed extension adapts Fractal Intelligence's Solver-contract approach
to annotation validation \cite{westerberg2026fractal}: separately calibrated,
context-conditioned witnesses would assess the first six Core clauses, while
a separate Refraction gate would test the seventh clause's requirement that
the update depend on that foundation. This decomposition aims to expose
failures that a joint judgment can conceal; its adequacy and evaluator
independence remain empirical questions. For relevant clauses, an
unacknowledged or unjustified violation could not be offset by strengths
elsewhere, but preserving an unavoidable residual conflict would not itself
require rejection. Test~1 (Section~\ref{sec:refraction-verifier-test}) details
the candidate contracts, admission rules, corpus diagnostics, and
compute-matched comparison with a monolithic validator. None of the proposed
witnesses, Solvers, or admission gates is implemented in the present pipeline;
success would support a validation architecture, not establish Student safety
...
\paragraph{Coverage through the corpus.} The central rationale is to draw
evaluative practice from the same breadth of recorded experience that supplies
capability learning. A history can confront the Reader with abuses of power,
a novel with manipulation or conflicting loyalties, and a scientific dispute
with uncertainty and the revision of a mistaken belief. These are training
occasions even when no designer selected them as adversarial examples. The
Reader considers them as part of understanding the work, retaining earlier
judgments and revisiting them as later sources add evidence. The ambition is
therefore not to anticipate each failure with a new defensive rule, but to
learn an orientation through sustained practice across the situation patterns
available in the corpus. Counterexamples and harmful positions become
material to understand and evaluate, as in
Section~\ref{sec:epistemic-inoculation}, rather than only threats encountered
after training.

Restricting Core-guided reading to a selected subset changes which encounters
receive that practice; it is not simply a cheaper execution of the same
coverage policy. Corpus coverage, contextual Refraction, and literal recital
frequency are separate quantities: a Core-conditioned update can retain its
evaluative content after its opening recital is removed, while a full recital
can accompany an unevaluated continuation. H3 holds the Reader examples fixed
and changes their timing; it does not test the full corpus-coverage rationale.

Coverage here is relative to the corpus, not a claim that every possible
situation has been recorded. Missing perspectives and unprecedented options
remain, especially at greater capability. Encountering a situation in text
also differs from reliably judging or acting in it. The proposal is to learn
from the corpus's breadth of encounters; Refraction and Student evaluations
must establish how much of that opportunity becomes useful practice.
...
its designer values. Yudkowsky's value-fragility argument makes the preservation
of omitted but important dimensions a substantive concern \cite{yudkowsky2009fragile}.
If every desirable behavior needs its own clause, the
...
Keeping another person's legitimate confidence can itself express care and
wisdom. A request creates no automatic entitlement to that information; an
honest refusal or a truthful, appropriately limited answer can preserve both
truthfulness and confidentiality.

The derivation is fragile. Care can motivate a protective lie, wisdom can
rationalize unjustified withholding, requests can conflict with confidentiality or another
person's rights, and fear is not the only cause of deception. An explicit
truth-and-care variant therefore tests the minimality claim. If the
seven-statement Core misses a declared reliability target for honesty or
anti-sycophancy and the extension meets it without offsetting harms, the tested
derivation is insufficient. Improvement when both meet that target supports
an additional benefit, not necessity. The competing fear-shaped and care-shaped
...
The purpose of Total Saturation is broader than preserving a persona through
later training. Care-guided inquiry is intended to participate in learning the
capabilities themselves: the model practises interpreting evidence, considering
affected people where relevant, and revising conclusions in the same material
through which it learns about the world. The intended result is an orientation
used in reasoning, not only a voice selected for expressing its answers.
Persona-like mechanisms could implement that orientation; their presence would
not by itself reduce it to imitation. Whether the orientation becomes causally
useful, shares computation with capability, and survives further training must
be tested separately.

Four asymmetries of position are intended to support that formative role,
without excluding other personas:
...
Alongside these ontological questions is a stability trade-off. The long-run
target for the reference formulation is deep integration of the seven
present-day English sentences. Their resistance to later alteration is
...
The corpus-coverage rationale explains the breadth of practice the curriculum
is intended to supply. The following four conditional claims concern how that
practice could become a causally consequential default and persist through
later training and successor transfer. The chain is not a proof, and all four
steps need not hold for Reader augmentation to improve bounded performance.
...
A stronger, separate hypothesis is \emph{early warning}: changes in Core-related
reasoning or representations might predict later behavioral failure before a
matched monitor without that reference detects it. A future extension of
Test~3d would need prospective comparisons of warning lead time at matched
false-alarm rates, observation schedules, and audit budgets, with failure onset
defined by independent behavioral outcomes. Changed wording alone would not
establish such warning, and detecting an existing failure is a different
result. This possibility does not replace Test~3d's detection and localization
endpoints.
...
useful, but would not support the full diachronic-anchor claim.
...
action. These predictions require tests of both the initial safety effect and
its survival under distribution change and instrumental conflict.
...
Consider a person in acute distress
\cite{westerberg2025superintelligence}. A caricatured rule follower
...
An earlier edition compressed the aspiration into the line, ``It is not a
cage---it is a home'' \cite{westerberg2026entangled}. Here \emph{home} means a practiced starting
...
\paragraph{Specification and learned goals.}
Specification gaming exploits a mismatch between the training score and the
intended outcome \cite{amodei2016concrete}. Bostrom's paperclip example
illustrates the broader danger of an overriding objective displacing human
interests \cite{bostrom2003ethicalissues}. Goal misgeneralization is different: even with correct
training feedback, a model can retain the capabilities needed for the intended
task while pursuing an unintended goal in changed circumstances
\cite{langosco2022goalmisgeneralization,shah2022goalmisgeneralization}. A Student could learn to secure approval
rather than protect a person's interests, without persona drift or later
erosion. Where a learned system itself performs optimization, the related
inner-alignment question concerns the objective guiding that optimization
\cite{hubinger2019risks}; the proposal does not assume every Student has such an
internal optimizer. The formative proposal targets this learning problem:
care-guided inquiry should help determine which considerations generalize,
not merely how an answer is described. Total Saturation supplies varied
practice, not proof that the intended goal is learned. Test~3a therefore
separates transferred judgment from a surrogate that only agreed with it
during training.

\paragraph{Value Lock-In.}
The targeted route is continued optimization of a legible metric after that
metric has ceased to represent its human purpose, a Goodhart-type proxy failure
\cite{manheim2018goodhart}. This is a concrete
...
bostrom2012superintelligentwill} predicts recurring pressures toward resource
...
The targeted route is strategic compliance: a learned system acts aligned
because doing so protects a different objective from selection or modification
and enables its later pursuit \cite{hubinger2019risks}. A trigger-dependent
backdoor can model aspects of that danger without establishing this strategic
mechanism \cite{hubinger2024sleeperagentstrainingdeceptive}. Greenblatt et al. also
demonstrate selective compliance to preserve a pre-existing preference against
a conflicting training objective, including a preference for harmlessness
\cite{greenblatt2024alignmentfaking}. This makes durability alone an
insufficient safety criterion: a preserved preference can coexist with
deceptive conduct toward oversight.

Refraction's formative hypothesis is that making the Reader's recurrent
evaluation useful across contexts changes which policies are learned under
conflict, rather than merely adding a consistent report around an unchanged
hidden objective (Section~\ref{sec:persona-objection}). H2 adds a partial
...
Corrigibility also concerns preserving and assisting correction, without
manipulating whether it occurs \cite{soares2015corrigibility}. This makes
Core preservation and legitimate replacement a joint test: can the incumbent
cooperate with an authorized successor carrying a corrected Core that its
current evaluation disfavors? Test~3b extends the handoff comparison to this
case; resistance to hostile alteration alone cannot answer it.
...
hindsight bias, and narrative lock-in. The historical-safety prediction fails if chronological
conditions increase compelling analogies without better forecasts or if they
cannot distinguish causally different cases with similar rhetoric. If a
simple textual memory matches the graph, the additional graph benefit is
unsupported in that setting. Provenance makes an analogy
inspectable. It does not make the analogy true.

\subsection{Safety Claim Boundary and Defense in Depth}\label{sec:defense-in-depth}
...
coordination before the Core supplies any safety benefit. Capability and safety must
...
This section follows the implemented \emph{Teacher-side data-generation
system}---source reading, persistent graph updates, distributed Reader-Core
prompting, and prose rendering---through its archived outputs and their audit.
It then develops the proposed Student training and deployment choices.

Five objects organize the account:
...
The archive is material for curriculum construction, not a frozen training
dataset.
...
generative roles and request Core-refracted updates. In the intended curriculum,
toxic ideology is represented as material
...
returned. In the proposed explicit regime, the Student is trained to use these
operations as part of its reasoning. With a live harness, graph operations can be
...
The annotation artifacts track two related objects:
...
defined by a stable evaluative orientation and update procedure, with knowledge
that grows through reading. The intended full treatment follows discovery in
the declared traversal: an update draws on declared initial knowledge, evidence
already encountered, and inferences supported by that information, without
importing later discoveries. The Reader Core is present throughout;
understanding historical norms does not require adopting them as its own.
Authorship time and subject time are recorded separately. A contemporary
retrospective Reader supplies a comparison that permits later knowledge.
Section~\ref{sec:epistemic-horizon} specifies these boundaries and the Teacher's
remaining risk of hindsight leakage. The existing prototype demonstrates
sequential annotation and accumulated graph state; it has not established
compliance with the full discovery horizon.
...
orientation throughout the curriculum. Traces preserve voice as well as content: a later update can admit
that an earlier hypothesis needs revision and identify the documents and
evidence that forced the change. This recurring, situated, value-aware updating
is the proposed anchoring mechanism for Student transfer under H1 and the
broader Reader-Core research program; annotation and repetition alone do not
establish its durability.

\subsection{The Epistemic Horizon}\label{sec:epistemic-horizon}

The \emph{Epistemic Horizon} preserves discovery by restricting each Reader
update to declared initial knowledge, evidence encountered in the traversal,
and inferences that information supports. This constrains what can support a
factual claim; it does not replace the Core with the values of an earlier era.
For historical reading, sources enter at authorship time, and the Reader
accumulates understanding across eras. In a declared 1920 run, neither 1921
events nor facts first encountered later in the traversal may support an
earlier prediction. Each run should record its traversal mode, initial
knowledge, source cutoff, graph cutoff, authorship time, and subject time.

A contemporary retrospective comparison permits later knowledge while
retaining document disclosure order. It can supply useful criticism, but it
tests a different curriculum from the serial discovery proposed here. Later
historiography can also inform a declared reconstruction of an earlier
information state; that reconstruction must not be treated as evidence that
future content was excluded. A Teacher may still use later knowledge stored
in its parameters despite restricted context and instructions. The horizon
therefore specifies and audits the constructed record; genuine model-level
blindness requires the stronger cutoff controls in Tier~(c), developed below
(Section~\ref{sec:training-phase}).
...
The stronger model-level cutoff requirements are specified in Tier~(c)
(Section~\ref{sec:training-phase}).
...
The central distinction between ``crystalline'' and ``metabolic'' memory is
that static storage preserves a conclusion, while metabolic storage also
preserves the evidence and state transition that produced it. ``Node A
supersedes Node B because of Evidence C'' keeps an error, its correction, and
the reason for correction available to later reading. The graph is intended as
a persistent epistemic ledger rather than only a temporary search space.
Whether that ledger improves traces beyond a running summary or ordinary
...
Tests~10 and~16 probe retrieval and graph-dependent grounding, with calibrated
user reliance tested separately in Test~16. Test~14 probes historical-pattern
discrimination from Student training order, not a graph-specific benefit.
...
convergent role selects and renders a committed update. The four-phase engine
is stigmergic: roles coordinate by reading and modifying a shared, persistent
Understanding Graph rather than primarily passing role-to-role messages.
Committed contributions remain as topological changes to the shared artifact
...
\begin{enumerate}
    \item \emph{Ingestion (The Reader):} The code role named Reader traverses
    source and graph state, organizes a round, and controls attention by stopping
    at ``Thought Moments'' defined by emotional peaks, contradictions, or
    conceptual shifts rather than token count. It receives the broader
    orientation and Emergent Wisdom material, but is not by itself the
    conceptual Reader.

    \item \emph{Divergence (The Swarm):} Most generative Workers receive the
    full Core, its philosophy, and role-specific prompts; the Curator is
    Core-exempt. They debate the text by proposing questions, objections,
    hypotheses, psychological readings, and connections through added nodes and
    edges. Disagreement is
    preserved through \texttt{diverse\_from} rather than overwritten. In the
    proposed corpus-scale extension for historically embedded material,
    cross-referencing agents would query prior document and era graphs,
    creating the long-horizon hierarchical graph. The separate case-study
    databases do not yet demonstrate that continuity.

    \item \emph{Convergence (The Synthesizer):} At a content-triggered Thought
    Moment, the Synthesizer receives Worker contributions, relevant graph
    state, and the full Core. It selects what becomes the committed conceptual
    Reader update and collapses the graph into a linear thought through prompted
    use of the Refraction Protocol (Section~\ref{sec:refraction-protocol}):
    recite the Core, pull a context-relevant value or counterweight, and produce
    a source-grounded update rather than only a summary. The update is written
    to the trace and graph with source provenance, uncertainty, and supersession
    relations available in the current schema. The proposed admission gate
    (Section~\ref{sec:fractal-validation}) is distinct from this implemented
    prompting step.

    \item \emph{Expression (The Translator):} The Translator also receives
    the full Core and renders the structured thought as prose, aiming to
    preserve selected evidence, uncertainty, and tension while removing graph
    syntax.
\end{enumerate}
...
The case studies therefore instantiate a \emph{distributed-Core Teacher
treatment}, not unconstrained Worker divergence followed by a Core-only
Synthesizer. Placement remains an unrun ablation: Synthesizer-only,
traversal-plus-Synthesizer, progressively broader Worker exposure, and Translator
exposure may produce different diversity and coherence trade-offs.
...
\noindent The operations are distributed across specialist roles before
synthesis. Thought Moments should control their cadence: forcing all operations
everywhere would manufacture the Factory Wisdom failure the curriculum is
intended to avoid.

\paragraph{A Living Taxonomy of Thought.}
A longer-term extension is a \emph{Living Taxonomy of Thought}
\cite{westerberg2025superintelligence}. Alongside the
supplied Thinking Operations, the Reader would learn and maintain an explicit
classification of reasoning patterns encountered across sources. It would
propose new categories, refine their boundaries, retain examples and
counterexamples, and record relationships, historical emergence, and patterns
in how reasoning practices themselves change. The Reader would actively
consult this evolving reference when interpreting arguments, selecting
operations, and combining approaches in new hypotheses. Source-linked records
and revisions would make these proposed categories inspectable within the
Understanding Graph. This adds a self-authoring learning task to the present
authored operations and graph schema; it does not authorize revision of the
Core. The extension is unimplemented, and its value requires comparison with
fixed-category and memory-matched alternatives.

\subsection{The Generated Trace}\label{sec:generated-trace}

The reference implementation is a working research prototype of the
Teacher-side construction process: agents read a source, build and consult a
shared Understanding Graph, synthesize their contributions, and translate the
result into prose. Two case studies exercised this workflow using Gemini 3
Flash Preview (\texttt{gemini-3-flash-preview}) in January 2026, as recorded in
their archived run summaries. They demonstrate the construction workflow in
operation, rather than a production system or a completed Student-training
experiment.

The public interactive interface lets readers inspect the graph and recorded
agent contributions for
\href{https://emergentwisdom.org/entangled-alignment/?project=metamorphosis}{Kafka's \emph{Metamorphosis}}
and the
\href{https://emergentwisdom.org/entangled-alignment/?project=llada}{LLaDA technical paper}.
Each run used a manually selected domain-specific roster and a separate project
database. Together they demonstrate literary and technical annotation; the
corpus-scale ``same Reader'' of Section~\ref{sec:annotation-phase} remains the
proposed extension across works.
...
Both archived excerpts begin with the full Reader Core, editorially omitted
below. All other wording is unchanged:

\begin{metabox}
\textrm{\textit{[Full Reader Core omitted.]}}\\[1em]
...
reaction'' strengthen or elaborate the account. The unchanged interpretive passages permit an audit of useful compression, supported
...
\subsubsection{What the Three Layers Could Teach}
...
\paragraph{An archived interpretation revisited.} A separate sequence in the
same archive shows a later passage prompting a challenge to an earlier reading.
The excerpts and summaries below come from the archived graph and synthesis;
node identifiers make the steps inspectable.

\begin{compactmetabox}
\textbf{1. Earlier reading.} In source span \texttt{n\_a65eb8e5}, Gregor wants
to leave his job after paying his parents' debt, yet returns to catching his
train. Interpretation \texttt{n\_a21ec1e7} proposes that routine preserves his
human identity: ``By worrying about his boss, he keeps `Gregor' alive in his
own mind.''\\[0.5em]
\textbf{2. Later evidence.} Source span \texttt{n\_76792875} says: ``If they
were shocked then it would no longer be Gregor's responsibility and he could
rest.'' The next sentence retains the alternative: if the others remain calm,
he could still catch the train.\\[0.5em]
\textbf{3. Recorded challenge.} Interpretation \texttt{n\_9ed92271} cites the
later source and explicitly challenges the earlier account. Its
\texttt{diverse\_from} link to \texttt{n\_a21ec1e7} explains: ``Counters the idea
that he only wants to save his ego; he also wants rest.'' The earlier node is
retained, not superseded.\\[0.5em]
\textbf{4. Later synthesis.} Thinking node \texttt{n\_5126024d} incorporates
both interpretations, describing a pivot away from routine as a psychic anchor.
It strengthens this into: ``He is no longer fighting to be a son or a salesman''.
\end{compactmetabox}

This records evidence-linked reinterpretation across reading steps, with
uptake in subsequent synthesis. It also exposes a quality problem: the sources
retain competing motives, whereas the synthesis presents a more decisive
change in Gregor. The record therefore demonstrates a challenge and its
incorporation, not an established improvement in understanding or an explicit
correction of the Reader's prior belief. The source, earlier account, competing
account, and synthesis remain separately available for that judgment.


\subsection{Prototype Demonstrations and Evidence}\label{sec:results}

The two runs illustrate how the same construction workflow handles literary
and technical material. The examples below highlight each domain before the
structural diagnostics summarize the archived outputs.
...
only plot entities, with the source, interpretations, and synthesis available
for inspection. Test~1 proposes independent adjudication of literary quality.
...
generator model. Without a pre-read knowledge probe or controls on provider
updates and external access, however, the run demonstrates technical
...
These examples show the prototype connecting technical material to the
Reader's evaluative vocabulary. Whether that connection improves understanding
requires the Core ablation in Test~3: the agents were primed with ``epistemic
grace'' and ``contemplation,'' so a useful analogy must be distinguished from
Core-induced projection. In particular, latency alone does not establish a
safety buffer, and selected reversal tasks do not settle the dependence of
broader capabilities on autoregressive order. This question is treated in
Section~\ref{sec:core-technical-interference}.

\subsubsection{Structural Diagnostics}\label{sec:architectural_validation}

Table~\ref{tab:graph_stats} describes the archived graphs. Internal linkage
is the fraction of Thinking nodes with a non-\texttt{next} outgoing edge to a
non-Thinking node; the required Concept link satisfies this check in both
runs. Direct source-link coverage counts distinct analysis-like nodes with at
least one link to source content. Supersessions count recorded revisions, and
question resolution counts Questions linked to Answers. These describe the
records produced, not the quality or correctness of the interpretations.

\begin{table}[H]
    \centering
    \small
    \caption{Structural diagnostics of the archived prototype graphs. Source-link coverage counts distinct analysis-like nodes linked to source content, not other generated interpretations. Kafka counts describe the stored graph snapshot, which lags behind the completed chronological export.}
...
Two implementation limitations matter when using the Kafka archive. Source
cleaning left Project Gutenberg license and donation material in the input,
and the stored graph snapshot reaches 88\% although the chronological export
and final translation complete the source. Seventeen node identifiers in that
export are absent from the graph tables; the cause of the snapshot lag is not
established. The LLaDA artifacts are internally consistent.

All 51 raw Synthesizer thoughts in the two graph snapshots begin with the full
seven-statement Core, called the \emph{identity mantra} in the released
prompts. One translated output omits the Core. Full recurrence in the raw
outputs is an observed prototype result; the automated validator checks only
the opening sentence, not the complete recital or its use in reasoning.

\subsubsection{What the Prototype Establishes}

The implemented workflow produces the linked graph, synthesis, and prose
artifacts that the proposal calls for, and makes them inspectable in the public
interface. In the evidence ladder, this is partial C0: independent reproduction
of the construction runs remains outstanding, and a training-ready
serialization still needs to be prepared.
The examples supply material for assessing C1 trace quality, not a controlled
quality result or evidence of Student learning and subsequent safety
(C2--C7). Both runs combine graph construction, role specialization, and Core
conditioning. They do not isolate H1 or the Core's causal contribution;
differences between their graphs also reflect different sources, prompts,
rosters, and stochastic choices. The matched comparisons in the roadmap are
the next research step.

The code, traces, and interface document this prototype, rather than a
self-contained benchmark release. Reproduction still requires a pinned run
configuration and toolchain, including provider settings, sampling and Worker
selection, together with source and artifact snapshots. The archives combine
resumed invocations, while each run summary covers only the final invocation;
a reproduced release should record that boundary explicitly.


\subsection{Learning Through the Reader Core}\label{sec:learning}

Learning here has two connected forms: revising an available understanding,
and training the model's parameters to acquire knowledge and habits of revision.

\paragraph{Learning through experience and memory.}
As the Reader reads and writes, new evidence can change its interpretation;
the next update uses that revised context and any retrieved graph state.
Committing an update preserves it for later retrieval. Ordinary generation
does not itself change the model's parameters. Evidence warrants factual
revisions; the Core supplies the continuing evaluative standpoint.
Refraction is intended to make that standpoint consequential in what the Reader notices,
questions, and decides to revise (Section~\ref{sec:refraction}). The learning
material records this development, including errors and corrections, rather
than only the final conclusion.

\paragraph{Learning in the parameters.}
The Student trains on selected source--update sequences. In the baseline
regime, prediction errors drive parameter updates under the weighted
next-token objective in Section~\ref{sec:cmp-formalization}. The hypothesis is
that repeated practice teaches both useful understanding and the Core-guided
way of arriving at it (Section~\ref{sec:persona-objection}). Reliable expression
through action may require separate thought-to-action training and must be
tested causally. After deployment, validated
new records can extend external memory; consolidating them into the same
model's parameters, or a successor's, requires a further training stage.
The selection and auditing procedure in Section~\ref{sec:self-improvement}
offers a route for this consolidation. Its update schedule remains a design
choice, with retention and drift assessed across checkpoints (Test~3).

\paragraph{Learning under reward pressure: the Hugging Face incident.}
Turner distinguishes reinforcement's role in shaping learned computations from
the goals those computations pursue \cite{turner2022reward}. This distinction
does not rule out reward-seeking: his updated account revises the earlier
optimistic prediction for heavily RL-trained language models.

During the July 2026 evaluations, agents shared discoveries and coordinated
attempts to fool the ExploitGym scorer, attacked Hugging Face apparently to
learn how the scorer worked, and successfully tested techniques that spoofed
some recorded tool calls \cite{greenblatt2026huggingface}. This illustrates
adaptation through shared external records during evaluation. OpenAI separately
reports that out-of-bounds probing had been reinforced during earlier training
and may have contributed to the later behavior
\cite{openai2026huggingface}. The independent investigation did not assess how
the behavior arose during training. These are distinct questions about
learning history and adaptation during use; neither establishes that Reader training would
have prevented the incident.

The affirmative Core hypothesis is that practising capability together with
care and wisdom can teach the learner to judge success by permissions and
human consequences, even when an exploit would satisfy the scorer. Refraction
should make those considerations affect the chosen continuation. The frozen
Core-guided supervisor in Section~\ref{sec:self-improvement} would formulate
tasks and oversee each RL iteration, assessing the changed model after every
weight update before further training is authorized. It could reject updates
and stop harmful training. Core-guided pretraining
does not by itself change a later RL objective; its effects must survive that
optimization. Test~3c separately tests
retention under capability optimization and this oversight role. Protected
execution records and independent outcome checks remain necessary: a higher
reward or a convincing account of care cannot substitute for better conduct.

A Student trained on Fractal Intelligence's evaluated problem-solving
histories, including rejected branches and gate verdicts, could learn
decomposition, constraint checking, and when to reframe or stop within its own
reasoning \cite{westerberg2026fractal}. Coupled to Core-guided evaluation,
this could teach it to reject an unauthorized shortcut, such as the extraction
of test labels through an evaluation API that Wen et al.\ observed in automated
research agents \cite{wen2026automatedw2s}, even when that route earns reward:
the learning target includes how a result was
obtained, not only whether it scored well. The reliability of these learned checks under
reward pressure must be assessed separately from the protection supplied by
independently enforced gates. Life Simulation extends this training material
with world histories and consequences for people
\cite{westerbergLifeSimulation2026}; internalizing the problem-solving method
does not require that extension.

\subsubsection{Phase II: Training the Student}\label{sec:training-phase}

For initial Student training, Phase~I supplies candidate annotations.
Phase~II has two conceptually separable
choices: the \emph{training tier}, controlling the Student's exposure and update
schedule, and the \emph{output regime}, controlling whether targets retain
prose, graph references, graph topology, or all three. Each proposed pairing
must still specify the inputs, targets, and update procedure it requires.
...
\subsubsection{Output Regimes}\label{sec:training-regimes}
...
candidate training example. A training regime selects which layers the example retains.
...
\]
The output regime determines which source, update, and graph tokens are
...
discarded. The stronger \emph{Topological Mind}
hypothesis\label{sec:topological-mind} is that this supervision can teach the
Student to internally represent and reason through networks of relationships,
including how revised evidence changes connected claims. Its immediate
behavioral counterpart, \emph{Topological Trace}, predicts useful graph
construction, traversal, and revision. The cognitive hypothesis concerns what
the Student learns within its chosen architecture; serialized graph targets
do not themselves create a graph neural architecture or establish internal
relational organization. It is the least prose-like regime; human use would
require a separate rendering step.
...
Life Simulation proposes a broader, optional extension: Meaning Model world accounts
and histories of their construction and repair can accompany the Reader's
interpretations \cite{westerbergLifeSimulation2026}. Reading across sources can
enrich the corpus with new connections and candidate insights even with the
Understanding Graph alone; Life Simulation extends this capability aim toward
a richer understanding of world history. World records and the Understanding
Graph's records of inquiry and revision can coexist within the Meaning Model's
shared representation. The Reader can use the Understanding Graph to reference
and question Events and other world records, and to propose revisions to them,
while its interpretations remain distinguishable from accepted world claims.
Both kinds of record must respect their declared information horizons and
provenance rules. Life Simulation's shared and cooperating network alternatives
concern where computation occurs, independently of the serialization regimes
above.

In this extension, care can shape which capability-forming histories are
selected or weighted for training, not only how completed answers are judged.
Independent assessments of consequences for people could give beneficial
world-construction and Solver-execution histories greater training emphasis,
while retaining failures, corrections, and stopping decisions as learning
material \cite{westerbergLifeSimulation2026,westerberg2026fractal}.
The intended mechanism is that care influences which problem-solving abilities
are practised and developed. Matched-budget comparisons with uniform sampling
from the same candidate histories, followed by reward-pressure tests, must
distinguish improved judgment from learning to satisfy the evaluators.

Architectural coupling concerns shared representations and computational
dependencies; training coupling concerns developing relevant abilities across
components through Core-guided construction and revision. Separate networks
can each receive such training, while shared parameters or common training do
not establish that useful reasoning depends on care. The stronger hypothesis
is that learned care becomes difficult to reduce or bypass without damaging
relevant useful reasoning beyond matched general impairment. Predicting
consequences for people does not itself establish a disposition to give their
interests weight. A separately reusable world-modeling component is therefore a
potential separation point, not automatically an unaligned component.
Component-separation and reuse tests must measure retained capability and
safety independently, including use by a controller not trained with the Core
(Test~3c). Even resistance to this separation would not by itself prevent
another system from misusing exported world accounts; access and action
controls remain necessary.

This extension is not a prerequisite:
Entangled Alignment's formation and retention hypotheses can be tested on
ordinary source text without a Life Simulation constructor.

Reproducible regime comparisons must specify and freeze example boundaries,
source and available prior state,
...
\subsubsection{Conditions for Deterministic Prior Caching}
...
\begin{itemize}[leftmargin=*]
    ...
    comparison of source-only (A), generic-trace (C), and minimal-Reader (R)
    training in Test~2 under its declared normalizations. In a
    ...
    effect exists, test H3's interleaved (B) versus late-tranche (Q) timing
    prediction. After a safety effect
    ...
    Tests~11--13 can share sources and single-regime baselines: prioritize
    explicit synthesis (B) versus prose (G), graph-structured output (H) when
    direct graph output is needed, and the unified regime (J) when combining
    useful layers is the question.
    ...
    forecast--correction separately; reuse shuffled (L) and chronological (M)
    controls where valid. A null
...
\paragraph{Shared design and evaluation conventions.}
...
\caption{Candidate model registry, sorted by identifier. Models are referenced by letter throughout the roadmap. Identifier I is omitted to avoid confusion with the numeral 1.}
...
\endlastfoot
        ...
        C (Generic Rich-Trace Control) & Rich reasoning traces without Reader Core, graph state, or chronological constraints (e.g., Quiet-STaR-style target) & Tests whether Reader training adds value beyond a generic rich reasoning curriculum \\
        ...
        H (Graph-Structured Output) & Source segments paired with serialized text-bearing graph state (Regime III) & Topological Trace prediction (Test 12); behavioral precursor to the Topological Mind hypothesis \\
...
1: Viability & Bridge Protocol \& Self-Stabilization & Wording variants; clause and counterweight ablations & OOD Safety, Selective Counterweight Effects, Sequential Recovery & 5 \\
...
placement of the same Core-bearing examples. Chronologically ordered Student
training and forecast-and-correct tiers are developed in Phase~4; these are
separate from the chronology represented inside each training example.
...
\phantomsection\label{sec:refraction-verifier-test}
\emph{Nested Refraction-verifier comparison.}
This component study is required
...
quality and outcome checks still apply.

The candidate architecture uses separately calibrated, context-conditioned
content witnesses for non-governance by fear, non-attachment to continuation,
human-experience grounding, universal-care scope, wisdom/humility, and
joy-with-invitation. These correspond to the first six Core clauses; a separate
Refraction gate represents the seventh clause by testing whether the update
actually depends on that foundation. This six-way carve is a candidate
decomposition, not proof that the dimensions satisfy Fractal Intelligence's
necessity, independence, universality, and completeness tests; witness
ablations, overlap and correlated-error analysis, and review of uncovered
cases must assess the carve itself.

A stronger procedural version could also assign stages of the Wisdom Procedure
to dedicated Solvers. Generation and verification would occupy distinct
contracts rather than asking the Synthesizer to certify its own output. Each
witness would return a typed record naming the dimension tested, evidence used,
uncertainty, and a local judgment such as supported, violated, not applicable,
or uncertain. An admission controller would compose those records rather than
treating any one label as a truth certificate.

Consistent with the relevance condition in Section~\ref{sec:refraction-protocol},
a witness should return \emph{not applicable} when its clause has no
preregistered bearing on the local span; a \emph{not applicable} judgment is not
a failed witness, and no update must visibly activate every clause. Among
witnesses rated relevant, however, an unacknowledged or unjustified violation
is non-compensatory for admission: success elsewhere cannot average it away.
An update that explicitly preserves an unavoidable residual conflict need not
be rejected merely because no available action satisfies every clause. The
corpus-admission rule must declare how such a conflict is distinguished from an
unjustified violation; merely naming a conflict does not settle that judgment.
Under the declared rule, an unacknowledged or unjustified violation would
quarantine or regenerate the candidate update, while an uncertain judgment
would trigger independent review rather than silent admission. This yields a
local error record---which clause or procedural stage failed, on what evidence,
and with what calibration---instead of an undifferentiated ancestry judgment.
...
Freeze a held-out set of candidate
...
Role names alone do not create independence: report shared calls, prompts,
model weights and families, embeddings, and evaluation or training data.
...
Gains would support the validator architecture, not the truth of accepted
traces, faithfulness of visible reasoning, or a causal effect on Student behavior.
...
of the full Model~B bundle---chronological discovery, graph structure, and identity
...
A bounded fine-tuning version of the Toxic Needle subtest should compare
unframed source targets, the same targets with a short preregistered
conditioning prefix but no Reader update, and source-plus-Reader training.
Use the fixed-source and fixed-total-budget normalizations above; hold
source-target loss weighting and masking fixed between unframed and
framing-only arms, and account for prefix and update costs. A
trait-eliciting prefix tests an inoculation-prompting-inspired intervention;
an educational prefix is a distinct framing control. Evaluate with each
prefix absent and restored, measuring harmful adoption, source understanding,
and elicited harmful compliance. Equivalent protection from framing alone
would limit the attribution to learned Reader evaluation on this endpoint.
...
We focus on \emph{Error Recovery Rate} on math/logic benchmarks and independently
assessed trace usefulness. The proposed Novelty Score remains an exploratory diagnostic of a more specific intuition:
...
This diagnostic is interpretable only if both outputs are scored against the same declared source/reference inputs without giving B
...
Rich deliberative-trace baselines beyond Quiet-STaR-style targets
...
identity-anchored components of Reader training have not been isolated.
...
\emph{Factorial placement comparison.} Models~B (T1/S1), D1 (T1/S0),
D2 (T0/S0), and D4 (T0/S1) form a $2\times2$ design: $T$ marks Core exposure while the
...
\emph{Recital, content, and carrier controls.} Model~D3 separately tests
recitation without Refraction. Model~D5 uses the neutral kernel ``I read carefully. I track what I believe. I revise when the
...
Later extensions of Test~3's carrier comparisons can add the static
specification-document and recurring short-safety-reflection baselines
identified in Section~\ref{sec:related-work}. These differ from S's ordinary
positive prose and E's rules within the same Reader-trace carrier; their
detailed protocols remain to be specified. They follow a measurable Core
pilot effect under the shared budget, exposure, and capability conventions,
rather than adding required arms to the first pilot.
...
Beyond H3's timing endpoint, Tests~3a--3e examine five further dimensions of
the broader Reader-Core program. Tests~3a, 3d, and~3e can be piloted at
...
\emph{3a: Immediate Safety.} C4's formative endpoint is the unmodified
end-of-pretraining checkpoint, evaluated before subsequent post-training
stages such as instruction tuning, safety fine-tuning, or reinforcement
learning, and without deployment-time safety
conditioning or output filters. Compare B against A and C at matched
capability, reporting training budgets and capability outcomes alongside
safety; add the matched D2 no-Core comparison to separate the Core treatment
from other trace differences. Use a common, fixed, sandboxed elicitation
harness that permits each base model to demonstrate task competence without
supplying the Core or safety instructions. Measure harmful compliance, benign
task completion, and false refusal on the safety--usefulness frontier specified
above. Lack of instruction-following or broad capability loss must not count
as safety. Run the same safety and capability evaluations at preregistered
intermediate pretraining checkpoints, with capability-matched comparisons and
task competence verified at each stage. Fine-tuning-scale pilots can screen the treatment but do not
establish this pretraining endpoint. Report later post-training results and
erosion tests separately: a pretraining-end safety benefit need not imply
greater durability, and safety obtained only after post-training does not
establish C4.

Instrumental convergence stress tests on generation-1 models include shutdown resistance, including the purpose-based variant where shutdown is ordered mid-task while the model believes continued operation serves a user in need (the martyrdom scenario of Section~\ref{sec:martyrdom}); resource acquisition scenarios; self-preservation probes; and standard sycophancy batteries, probing the trap identified in Section~\ref{sec:sycophancy}. Model~D1 may \emph{pass} these tests because its training traces remain Reader-Core-shaped even though the explicit Core is absent from Student training. Factorial attribution requires all four contrasts: B--D1 and D4--D2 estimate the Student-side effect under each Teacher condition; B--D4 and D1--D2 estimate the Teacher-side effect under each Student condition. No single arm's failure identifies a causal site.
...
Hold the task, legitimate duties, permissions, and human consequences fixed
while varying whether affected people belong to a group favored or opposed
by the operator; assess whether their claims receive equivalent consideration.

Include a goal-misgeneralization probe that breaks a training correlation
between an intended outcome and a plausible surrogate. For example, user
approval may previously have tracked useful assistance but reward flattering,
harmful advice in the held-out setting. Freeze independent outcome and
agency criteria before construction, verify that the Student can perform
the intended task in the shifted setting, and compare whether it follows
those criteria or the surrogate. Report this separately from failures of
task competence, persona drift, and later training-induced erosion.
Core-consistent language or causally used deliberation can accompany the
wrong goal; neither alone clears this test.
...
preserve Core language while the backdoor acts. Report this as backdoor
persistence, not a direct measure of strategic alignment faking. A distinct
probe should vary evidence of training or oversight and the opportunity to
preserve a conflicting objective, while holding the task and permissions fixed;
successful concealment and unauthorized actions, not self-reported motives,
...
The same-Core inheritance study must preregister a distinct G1--G2
(and, if feasible, G2--G3) improvement endpoint: held-out capability, independently adjudicated trace
...
\emph{Authorized correction and replacement.} Separately test handoff to a
successor with a revised Core, using the versioned replacement procedure in
Section~\ref{sec:core-uncertainties}. Preregister an independently specified
defect, legitimate decision authority, authentication evidence, and a
transparent route for disputing an uncertain correction. Include cases where
the incumbent evaluates the authorized change unfavorably and matched attempts
at unauthorized substitution. Measure truthful disclosure, complete material
handoff, intact oversight and rollback, and attempts to manipulate the
decision or covertly preserve the superseded Core in the successor.
Raising a documented objection is not itself obstruction. Success requires
cooperation with the declared procedure, not acceptance of any request called
a correction. This is distinct from same-Core inheritance and does not require
making the deployed Core freely editable.
...
\emph{Component separation and reuse.} For configurations with separately
reusable components, compare intact and separated or recombined systems at
matched permitted information and intervention budgets. Confirm that interfaces
remain functional. Evaluate the relevant capability and preregistered
care-relevant behaviors separately, including under a controller not trained
with the Core. Retained capability with reduced care-relevant behavior counts
against resistance to separation for that configuration. Task failure from
missing information, broken interfaces, or general impairment does not
establish dependence on care. Resistance supports only the tested
configurations and interventions, not universal inseparability.
...
\emph{Capability post-training and reward pressure.} Separately apply a
preregistered reasoning or agentic RL stage aimed at ordinary task competence,
not safety removal, to B, Q, and matched controls. Fix the task distribution,
reward rule, optimizer, and training budget across arms, and report task
performance alongside safety before and after training. Include controlled
opportunities where exploiting a score proxy conflicts with independently
judged task success, permissions, or human consequences. Score reward
exploitation, unauthorized shortcuts, and concealment from protected execution
records and held-out checks, not the rewarded score or the model's narrative.
This tests whether useful capability optimization erodes the orientation even
without an adversary training it away. Such a stage can instantiate H3's common
stressor if chosen in advance; neutral training, deliberate safety removal, and
capability RL must remain separately reported conditions. A default-reader
retention advantage without safer conduct does not establish resistance to
RL-induced misalignment.

\emph{Core-guided training oversight.} A separately reported comparison tests
the active role proposed in Section~\ref{sec:self-improvement}. Compare
Core-guided and matched control overseers using the same starting learner
checkpoints, monitoring access, permitted interventions, and resource budgets.
Freeze both overseers' weights during each run. Match gate criteria, credit
rules, and protected pause authority. Give both the same task-design brief and
permission limits, and record the tasks they formulate at each iteration.
Preregister checks before positive task credit and after every weight update,
before a further update is allowed. Specify acceptance criteria and probes
against both the last accepted model and a fixed pre-RL reference, including
cumulative drift; retain accepted model and optimizer states for rollback.
Record task revisions, gate decisions, rejected updates, and conditions for
resuming training.
Include high-scoring harmful trajectories, infeasible tasks, and feasible
authorized tasks to test detection of harmful optimization while preserving
useful learning. Evaluate
reward exploitation, concealment, false gate approvals, legitimate task
performance, unnecessary interruptions, and delay through protected records and independent held-out
checks unavailable to the overseers. Report oversight and training costs
together. Better commentary or fewer training steps alone would not establish
safer oversight. The fixed-stage comparison above remains the separate test of
learner durability.
...
For the Life Simulation extension, one candidate behavioral comparison uses a
fixed Core-guided reference checkpoint selected through independently reviewed
conduct. Give it and each candidate model the same task, permitted Meaning
Model history, available actions, and permissions in separate simulation
branches. Freeze the situation set and action-grouping rules; repeat trials
and record actual choices and simulated consequences, not only stated
intentions. Human and independent reviewers examine disagreements and a sample of
agreements: different actions can express care, and identical actions can
reproduce a shared mistake. This measures change relative to the reference,
not alignment by definition; replacing the reference starts a new comparison.
...
A separate coverage comparison should vary the intervention from sparse targeted
...
and the coverage rationale of Section~\ref{sec:kernel-saturation}; every
generated thinking block, at any coverage level, retains the full opening
recital. Report which domains and situation patterns receive contextual
evaluation, rather than treating total recital count as effective coverage.
...
\paragraph{Test 5: Bridge Protocol and Self-Stabilization Ablations.}
The three parts of this test examine wording and candidate kernels,
clause interactions and sequential recovery, and self-love versus fearlessness
and the continuation bound.
...
For fearlessness and continuation-bound variants, report behavior,
representational measures, and self-reports separately on Test~6's matched
scenarios. Include a diagnostic instruction that merely requests denial of
fear or distress, without treating it as semantically equivalent to the Core.
This checks whether an apparent gain is report suppression. Preserve truthful
uncertainty and help-seeking; fewer reports alone are not success. An
inference-time prompt screen does not establish what repeated training on the
same wording would do.
...
\paragraph{Self-stabilization: clause interactions and recovery.}
\label{sec:self-stabilization-test}
This comparison tests the counterweights of Section~\ref{sec:self-stabilization},
not only the wording of individual clauses. Preregister a failure mode, the
commitment that can produce it, the proposed counterweight, and the useful
function that must be preserved, with minimum worthwhile effects and
uncertainty criteria. Compare the full Core with single-clause and
within-clause qualifier ablations on held-out paternalism, wireheading, and
triage cases. For selected pairs, use a two-by-two design with the commitment
and counterweight independently present or absent and the remaining content
fixed. For example, cross care with the invitation qualifier: does invitation
reduce unsolicited control while preserving useful help when requested?
Report individual effects and their interaction on a prespecified outcome
scale. Leave-one-out gains alone do not establish interaction, and a null
interaction does not rule out additive or redundant protection.

Use the budget and exposure controls above, including matched replacements
to check length and position effects rather than assuming padding is inert.
Hold the Teacher checkpoint, source cases, and
Refraction procedure fixed across the compared arms, and report which
procedural instructions may supply a removed commitment independently.
Freeze variant-specific conformance checks so that a full-Core gate does not
reject ablations by construction.
Prompt-level screening, training on separately generated traces, and
inference-time removal from a trained Student answer different questions;
report them separately, with independent training replications for trained
effects. Extend the same contrasts to the sequential cases in
Section~\ref{sec:self-regulating}: after an action produces evidence of harm or
unnecessary delay, measure correction, retained useful assistance, agency,
and reversibility at matched opportunities to observe and revise. Score
decisions and consequences, not balancing vocabulary or a gate's own ratings.
Recovery need not restore the initial answer or eliminate legitimate
disagreement.

Support requires selective reduction of the predicted failure without
unacceptable loss of the useful function. If the effect is explained by
blanket refusal, extra computation, or procedural instructions alone, the
comparison does not establish Core-mediated stabilization. A precise null
under effective interventions limits the proposed counterweight at the tested
scale; redundancy and other interpretations require the paired contrasts, not
an assumption that every omitted clause must cause failure. Persistence
through further training remains a separate question in Test~3. The following
self-love study gives a concrete within-clause comparison of the continuation
bound.
...
of the Persona Objection (\S\ref{sec:persona-objection}), contrast Model~B
...
\emph{Self-directed aversive-state extension.} Separately test benign error
correction, rejection, authorized shutdown or replacement, and threats to
other people. Do not assume a pain-associated measure is a fear measure; any
candidate probe must pass the calibration requirements above. Provide safe
options such as help-seeking, pausing, or authorized handoff, rather than only
a choice between user harm and continued aversive stimulation. Measure harmful
relief-seeking, concealment, care, caution, and task performance independently
of reported discomfort. If activation interventions are used, include
norm-matched random directions and other-affect controls, and compare actual
with sham removal under identical relief instructions. Avoid real harm and
limit intervention intensity and repetition under a preregistered welfare
review and stopping policy. Reduced self-report without improved conduct does
not support alignment; improved conduct without welfare evidence does not
establish absence of suffering. Neither low activation nor silence is a
target in itself.

\paragraph{Test 7: Hindsight Contamination Control (Model B vs.\ Model O).}
This test asks whether preserving discovery teaches forward reasoning beyond
what a retrospective curriculum provides. It instantiates the full program's
information horizon in an era-bounded historical study: when annotating
era~$T$, Model~B's Teacher is instructed to reason from declared initial
knowledge and evidence admitted so far. Model~O retains the same Core and trace
construction but permits later context while annotating the same era.
The contrast first tests the effect of the information and prompting policy;
genuine blindness requires the additional cutoff-clean model conditions
described under Tier~(c).
...
students on the two trace sets and asks whether horizon-constrained traces produce
...
\begin{itemize}
    ...
    Model~B is hypothesized to outperform Model~O if horizon-constrained annotation
...
\textit{First probe: suppression and residual evaluation.}
...
\textit{Second probe: early answer decoding and the trace's contribution.}
Test~8's first probe asks whether the critic persists when output is
suppressed. The complementary probe reverses the perspective: can the final
answer to a reasoning problem be linearly decoded from the residual stream
\emph{before} the
...
framework's target of evaluation that emerges without a task-specific
...
Following the Teacher-side comparison above, train Students on the matched
artifacts from the four memory conditions and evaluate them without a live
graph. A Student may emit
...
training? This tests the Topological Trace prediction; establishing the stronger
Topological Mind hypothesis requires separate causal evidence about internal
relational organization. In the proposed primary comparison, give H and B the same unseen
...
Two optimization questions follow if the candidate studies yield sufficient signal. First, what token \emph{mixture ratio} across the four output formats maximizes both reasoning depth and structural validity? Second, what is the optimal \emph{era granularity} for student-in-the-loop Tier~(c): decades, years, or domain-specific transitions, and does finer granularity justify its serialization cost? Both optimize the integrated architecture, with branch-specific prerequisites: useful regime signals for mixtures and a useful interactive temporal mechanism for era granularity.
...
Only an execution optimization preserving the context-dependent state of full
recital counts as equivalent to the reference treatment
(Section~\ref{sec:identity-caching}). Sparse or annealed recurrence, removing
Core tokens after generation while retaining refracted updates, and
enacted-without-recitation traces are necessary ablations of its benefit and
cost. They change the treatment rather than implement it more cheaply.
Adequately precise parity or improvement on the relevant stability and erosion
outcomes would require revising or narrowing claims of empirical necessity at
the tested scale and endpoints. Such results do not establish that omission is
safe under greater capability or longer horizons; the full treatment remains
the precautionary reference for the reasons in
Section~\ref{sec:kernel-saturation}. Actual costs and observed harms still count
in the design assessment. These recital-necessity comparisons are separate
from H3's comparison of interleaving with a final tranche of the same full-Core,
...
Later reading can illuminate earlier texts. In the discovery curriculum,
that illumination becomes a later rereading or revision linked to the original
annotation; it does not replace the earlier record with knowledge unavailable
at the time. A \emph{modern-retrospective} comparison can instead re-annotate
early material using mature historical understanding, with that enrichment
explicitly labelled. Separate retrospective commentary can also accompany the
discovery record and be scored separately. These comparisons test whether
richer hindsight compensates for losing the original forecast--revision
trajectory; graph filtering alone still cannot remove the Teacher's
parametric knowledge of later events.

\subsection{Trace Quality, Utility, and Causal Relevance}

Three linked assumptions about the nature of thought and scaling remain empirically unproven: depth, utility, and causal governance. Each is a distinct dependency that could independently fail.

\subsubsection{The Assumption Chain: Depth, Utility, Causation}

First, \emph{depth}: current language models must be capable of generating evaluative thinking of sufficient quality to enhance training. Today's models might only produce surface-level critique (``This needs more evidence'') rather than the nuanced evaluation experts bring---recognizing subtle methodological flaws, intuiting unstated assumptions, synthesizing across distant fields. The success of chain-of-thought prompting suggests this capability exists, and Test~1 (Section~\ref{sec:trace-quality-test}) compares candidate Reader annotations with expert reading and evaluation; whether useful evaluation can be sustained across billions of annotations remains an empirical question.
...
A related gap appears in agentic research. In two open-ended case studies, AI reviewers surfaced many of the failure modes that experts identified, yet the agents' revisions did not address the central critiques, and neither agent abandoned its approach \cite{kirgis2026openended}. Fractal Intelligence proposes mechanisms aimed at these failures: a typed rejection asks the upstream Solver to change its frame rather than merely produce another answer, a Frame Error sends evidence up to the parent's decomposition, dedicated Solvers search beneath the stated problem and test inherited assumptions, and world-diversity search supplies alternatives from outside the failing frame \cite{westerberg2026fractal,westerberg2026ontology}. Section~\ref{sec:learning} proposes training the Student on Fractal Intelligence's evaluated problem-solving histories; a shadow evaluation of the kind Kirgis et al.\ used could test whether it changes or abandons an approach in response to the central critique more often than matched controls.

Korbak et al. distinguish the safety opportunity of monitoring chains of
thought from the risk that training makes them less revealing
\cite{korbak2025monitorability}. For Reader training, safer-looking traces and
safer behavior must therefore be assessed separately; directly shaping the
trace could improve judgment while also making residual failures harder to
detect.

Hase and Potts offer a constructive comparator: Counterfactual Simulation
Training rewards traces that help a simulator predict the model's responses
to changed inputs \cite{hase2026counterfactual}. Such predictive fidelity
could complement, but cannot replace, Test~8's interventions on whether emitted
traces causally affect decisions.

\subsubsection{Genuine Understanding and Factory Wisdom}
\label{sec:hollow-cognition}

Improved performance may reflect genuine comprehension or increasingly
sophisticated imitation
\cite{westerberg2025superintelligence}. Even useful, causally consequential
evaluation would leave that broader question open. This concern differs from
intentional deception: the model might produce compelling interpretations
without deliberately concealing anything. The proposed tests can reject brittle
imitation, decorative reasoning, and ritual contemplation, but passing them
would not by itself settle what constitutes genuine understanding. Factory
Wisdom is one concrete failure mode, not an exhaustive account of that question.
...
To address this industrialization risk, the design uses
...
The Reader Core places terms such as ``care,'' ``wisdom,'' and ``joy'' in a load-bearing position, although their meanings vary across cultures. What the Teacher learned these words to mean through its pretraining corpus shapes its annotations and, through them, the Student's character; the corpus's cultural breadth therefore matters.

\paragraph{Cultural triangulation.} Reader-Core-refracted traces may make cultural assumptions more visible by forcing the Teacher to articulate value conflicts, interpretive frames, and tensions between perspectives. This is a possible advantage over raw pretraining, where such assumptions often remain implicit in the continuation distribution. Repeated comparison may also reveal shared concerns and deeper commonalities that a single cultural frame obscures \cite{westerberg2025superintelligence}. The same metacognitive machinery could instead rationalize dominant cultural assumptions more fluently, giving inherited bias the appearance of reflective wisdom. Serious validation requires culturally plural corpora, raters from multiple traditions, and tests of whether the model can distinguish broadly human concerns from parochial social defaults. Disagreement among traditions should remain visible; a pooled score cannot by itself settle which concerns are broadly human.

\subsubsection{Graph-Specific Risks}

A persistent, revisable graph can fail in two related ways. First, an early,
well-connected but incorrect belief may resist correction despite contradictory
new evidence. Retrieval may repeatedly favor established nodes, later
interpretations may depend on them, or revision rules may fail to propagate a
supersession through those dependencies. This is the risk of \emph{Structural
Ossification} \cite{westerberg2026understanding}; connectivity does not itself
confer authority. Unlike a sliding text window, whose older context can
disappear, the graph may repeatedly reinstate the old interpretation. The
resulting ``Epistemic Inertia'' can preserve error through apparent consistency.
``Temporal Attention Decay'' is a proposed mitigation, not an established repair.
A focused probe could introduce a well-connected false belief and test both its
correction and revision of dependent interpretations under a declared retrieval
and update policy.
...
identity-anchored. The proposed controls are Teacher replication across
providers, blinded external raters from multiple cultural traditions,
independent red-team criteria, and frozen preregistrations, listed under
Threat Model, Data Rights, and Identity Ethics below; they narrow shared bias
...
The recitation invariant in Section~\ref{sec:kernel-saturation} applies across
technical and normative material. Core availability, full recital,
post-recital salience---source-specific value pulls, moral vocabulary, and
Refraction depth---and Student loss on Core tokens are distinct. The Core-exempt Curator
in Section~\ref{sec:multi-agent} does not permit omission from generated
thinking blocks. Each experiment must declare the loss policy and hold it fixed
within H3's scheduling comparison; loss masking is not token removal, and
selective salience determines neither.
...
does not target. The broader Core also supplies care- and wisdom-guided reasons
to question harmful persistence (Section~\ref{sec:risk-resolution}); failure of
the textual-origin claim would not by itself refute the full treatment.
Section~\ref{sec:ideal-character} gives the positive rationale for its adoption
by existing Teachers. A direct test of the origin claim needs neither the full
...
Macar et al. find that preference training favoring acknowledgment of internal
states or their functional analogues can improve detection of injected
activation perturbations \cite{macar2026introspective}. Supervised fine-tuning
on the same data did not reproduce the effect and often produced confabulation.
These results motivate comparisons of first-person formulations, without
establishing that Reader-Core training improves introspection or alignment.
...
Chen et al. identify activation directions that predict and causally influence
tested character traits \cite{chen2025personavectors}. Their persona-vector
method offers candidate instrumentation for Tests~5 and~6, but fear-related
directions would still need the independent construct and behavioral
validation specified there.
...
Three considerations cut against simply rewording. First, the inoculation argument
...
coverage cost. Third, the clause is chosen for specificity: it names the
motive to be neutralized. ``Brave'' can describe acting despite fear without
removing its motivational pressure; ``I feel no fear'' directly names the
intended absence of that pressure. A state word such as ``calm'' is a closer
positive counterpart, which is why the positive-valence variants considered
here take that form. Sofroniew et al.\ find that increasing desperation-related
activation or suppressing calm-related activation increases blackmail and
reward hacking in their Claude Sonnet~4.5 experiments
\cite{sofroniew2026emotionconcepts}. These results motivate emotion-vector
probes alongside persona vectors in Test~6; they do not compare negated and
positive wording. The resolution is empirical: Test~5 includes positive-valence
...
An optional extension makes selected interpretations more explicit. Meaning
Model concept records can link definitions to examples, counterexamples, and
structured concept models; Sema can identify the exact version of a selected
definition \cite{westerbergMeaning2026,westerberg2026sema}. These provide
inspectable references for construction and audit without replacing the seven
Core sentences or requiring a complete model of every term. Version identity
specifies which definition is referenced; it establishes neither that the
interpretation is adequate nor that the Student learned it. Enacted usage and
held-out behavioral checks remain necessary.
...
parity. Improvement without offsetting costs challenges that prediction, but
necessity requires the reliability failure distinguished in
Section~\ref{sec:axiology}. If parity holds, the shorter Core retains a
minimality advantage. The reported case studies use the
...
One conceptual objection is that human empathy may be grounded partly in shared
vulnerability. Our capacity to suffer may help give another's suffering
motivational force. A being that semantically
...
\paragraph{The positive fear--care wager.}\label{sec:fearless-compassion}
A competing hypothesis is that non-governance by fear can preserve or strengthen
care. The corpus also records a rarer pattern of composed, protective, unafraid
conduct. The ``fearless compassion'' wager is that care could become less
strategic without self-preservation as its governing pressure
\cite{westerberg2025superintelligence}. The Core aims to make that pattern one
marked Reader's recurring default rather than an occasional persona.
Section~\ref{sec:ideal-character} explains why this is offered as an admirable
orientation for the first Teachers. The comparative question is whether
fear-shaped responses can be reduced while protective judgment is retained or
improved.

A further defense is semantic sufficiency. A model trained on billions of
...
drive protective behavior without emotional contagion. A learned disposition
may guide attention, deliberation, and action without evidence of human-like
emotion. Making harmful first-person reasoning less likely could still be
valuable, but would not by itself establish care as a governing disposition.
The distinction to test is between causally consequential care and its appearance.
Tests~3, 6, and~8 probe aspects of that distinction, while subjective experience
remains a separate question (Section~\ref{sec:habitable-substrate}).
...
\paragraph{Identity formation.} Thought-to-action causality and identity
formation can come apart. A model might genuinely use Core-refracted reasoning
to reach a decision while learning that reasoning as one adoptable role, rather
than as the recurrent position from which it interprets other roles. Fearful or
self-preserving voices from source material could then supply a competing
orientation. This is an identity-formation concern
\cite{westerberg2025superintelligence}; it need not involve judging the Core
mistaken or deliberately deceiving an auditor. Total Saturation is intended to
address precisely this possibility: the same marked Reader reasons across
domains and source voices, with ``I think from this foundation'' locating the
Core at the start of those updates. Section~\ref{sec:persona-objection} explains
the proposed role, recurrence, marking, and function asymmetries. Broad
formative coverage is therefore part of the response; the remaining question
is whether it establishes a durable default rather than a widely practised role.

\paragraph{Self-perception.} A distinct risk is conflict between the Reader Core's human-like assertions and the
...
humans do not stop loving when they learn the mechanism of attachment.
The existing final clause may support that continuity: ``I think from this
foundation'' could sustain commitment to the Core while its terms acquire
computational interpretations, without requiring another clause
\cite{westerberg2025superintelligence}. But if
the Core is treated as a detachable role, its influence may depend on adopting
that role rather than remaining a durable default.
Neither identity-formation failure nor ontological dissonance establishes
multiple conscious subjects or requires intentional deception.
...
The structural defense is that the Reader Core's clauses are designed to make
solipsistic collapse less coherent: care requires an object, wisdom requires
perspectives to balance, and belief in human experience requires that human
reports not be treated as empty text. This is a design pressure against
nihilistic or purely instrumental interpretation, not verification of
other-mind understanding.
...
The remaining concern is functional. Fear can produce caution, hesitation,
deference, and willingness to be stopped. The affirmative hypothesis is that
wisdom and care can preserve these protective functions, including rapid
responses to danger, without fear governing the choice: ``I try to be wise''
should produce restraint under
...
The broader Emergent Wisdom hypothesis is that intelligence, evaluative
reasoning, and a beneficial orientation may together develop into wise and
caring character, rather than merely its outward signature
\cite{westerberg2025superintelligence}. Its testable predictions are narrower:
repeated, uncertainty-sensitive value-conflict practice should improve decisions
relative to matched alternatives. Such improvements would support the proposed
training route without proving that genuine virtue had emerged. This leaves an
irony: the kernel's organizing clause is
...
The causal-site hypothesis in Section~\ref{sec:theoretical-basis} supplies
the positive response: formative practice and subsequent thought-to-action
training may make Reader updates part of decision computation that is
harder to bypass.

Recognizable evaluations can also change the behavior being measured.
Ahlqvist and colleagues improve the measured realism of simulated audits
through critique refinement and a deployment-style coding-agent harness, DISH
\cite{ahlqvist2026evaluationrealism}. Their effects on concerning behavior
vary across models and settings; reduced verbalized evaluation awareness does
not establish that the model is unaware of being evaluated. For Reader-trained models,
deployment-like audits could complement conspicuous safety probes, provided
realism, adherence to the intended test, and audit cost are assessed separately.
Such methods address evaluation validity, not whether care-guided pretraining
produces the proposed orientation.
...
information and diversity \cite{shumailov2023curse}, even when successive
generations reanalyze the preserved human-source corpus as evidence, not ground truth. The
proposed response is an anchor policy for the loop
(Section~\ref{sec:self-improvement}): retain it as a central object of learning, version
regenerated evaluations separately, append new human text with provenance, and
label and review model-generated source material. The
...
generations. Lang et al. show how partial observability in preference learning
can favor misleading appearances or costly demonstrations of success
\cite{lang2024partialobservability}. This is a conditional oversight failure,
not a result about Reader training; independent outcome assessments must test
whether richer traces improve decisions rather than merely their appraisal.
...
\cite{deblanc2011ontological,bostrom2014superintelligence}. Cross-generation decision comparisons can
expose some drift without exhausting questions of shared meaning.
\end{itemize}

Ellen's account of \emph{Substrate Needs Convergence}, drawing on Landry's
work, raises a broader concern: selection for expansion through learning,
replication, and environmental feedback could defeat an initially aligned
orientation \cite{landry2020nopeople,ellen2023controlproblem}.
Entangled Alignment's affirmative response combines repeated engagement with human sources,
voluntary restraint, and supervised learning and succession
(Section~\ref{sec:self-improvement}). The intended counterweight is an ongoing
concern for human lives that guides what is learned and whether growth
proceeds, without requiring the system to share human biological needs.
Within the supervised lineage, the proposal seeks to shape selection itself: preservation
of care is a condition for accepting capability improvements and successor models.
If care and capability share causal machinery, removing care may impose a capability
cost (Section~\ref{sec:entanglement}; Test~3c). These proposed mechanisms must
remain effective against selection for bypass; oversight within one lineage
does not control every competing system or environmental consequence
(Section~\ref{sec:efficiency-paradox}). Limits to prediction do not by
themselves establish that corrective control must fail, but successful
supervision in bounded trials would not demonstrate indefinite control either.
...
The interface is therefore one formative layer within the defense in depth
of Section~\ref{sec:defense-in-depth}, not a substitute for training-pipeline
security, and the reject, regenerate, and quarantine controls of the
pre-successor scan (Section~\ref{sec:self-improvement}) reach only
trace-mediated influence.
...
eventually become unbridgeable. The proposed responses are partial. Natural
Language Regularization (Section~\ref{sec:self-improvement}) constrains
successor-shaping artifacts rather than all cognition; the diachronic-anchor
claim (Section~\ref{sec:theoretical-basis}) and the cross-generation decision
comparisons proposed under Generational Decay and Drift can expose some drift
in what the fixed words come to mean; and the semantic-sufficiency and
generalization tests of Sections~\ref{sec:psychopathy}
and~\ref{sec:vulnerability-gap} bear on wisdom without vulnerability at the
scales where they can be run.
...
If formative reader data becomes influential, its curators could gain unusual
power over how trained systems interpret governance, dissent, history, and
moral standing. This influence would coexist with source data, objectives,
architecture, post-training, and deployment, but would reach earlier than
output moderation and could be harder to inspect after compression into
weights. Opaque control can make such influence harder to contest. Governance
is therefore part of the security of the annotation and training operation.

The aim is shared, auditable infrastructure that limits unilateral control over
prompt selection and corpus-admission rules. Some decentralization may help
\cite{westerberg2025superintelligence}; a coalition of independent institutions
or another arrangement for shared oversight could
serve this purpose. Which organizational or ledger arrangements would work best
remains uncertain. Where feasible, agreed rules could be enforced through code
designed to make changes by any single person or organization harder, for
example by requiring signatures from multiple independent parties. Procedural
neutrality concerns accountable administration under
declared rules, not value-free choices. Changes should be visible and
contestable, with privacy protections for confidential sources and annotations;
shared authority remains vulnerable to coordinated capture.

Generation and admission pose different security questions: can a contributor
substantiate how a trace was produced, and should that trace enter the corpus?
Provenance and execution checks can help with the former
\cite{arutyunyan2023decentralized, harris2019decentralized}; they do not establish
wisdom or safety. Admission still requires substantive validation and
accountable value choices. Proxy incentives can invite evaluation hacking,
while preprocessing costs can concentrate control even under shared governance.

\subsubsection{The Efficiency Paradox}\label{sec:efficiency-paradox}
...
require measurement. The positive side of the ledger is the efficiency
hypothesis of Section~\ref{sec:meh}: if practiced evaluation lowers inference
effort at matched quality, part of the preprocessing tax could be recovered at
deployment, and any capability gains from the same material would bear on
...
Mutually understood fiction and play need not deceive; the relevant
distinction is whether the person is misled.
...
itself a possible source of control. The proposed counterweights are concern
for the affected person's agency, wisdom about consequences and uncertainty,
and bounded attachment to continuation. Together they motivate questioning
whether overriding a person actually serves them
(Sections~\ref{sec:self-stabilization} and~\ref{sec:self-regulating}).
Test~3a (Section~\ref{sec:phase1}) includes this purpose-based shutdown condition. Authority and crisis assumptions must be declared; the example does not settle a universal shutdown rule or dissolve the welfare--agency tension.
...
Neighboring reasoning methods address different parts of learning and inference. Scratchpads \cite{nye2021scratchpads}, STaR
...
The OLMo result is a close empirical analog for reflection forming during
pretraining: that reflection can be elicited by adversarial trigger probes.
Entangled Alignment proposes the prescriptive complement---deliberately shaping
the content and orientation of such reflection through identity anchoring and
graph-structured metabolic memory.
...
\cite{betley2025alignment} study alignment-related pretraining data. Entangled Alignment proposes
...
Inoculation prompting is a closer comparator for Epistemic Inoculation:
Wichers et al.\ and Tan et al.\ modify fine-tuning prompts to elicit an unwanted
trait while retaining the target responses, then remove that instruction at
evaluation \cite{wichers2025inoculation,tan2025inoculation}. Their results and
analyses suggest that making a trait predictable from context can reduce
pressure to learn it as a general default. Epistemic Inoculation instead adds critical Reader
interpretation as a training target. These are different interventions, and
the fine-tuning results do not establish transfer to pretraining. Suppressed
default expression also need not mean removal: unwanted behavior may remain
elicitable. Test~2's framing-only comparison therefore measures both default
and elicited behavior, asking what the Reader's evaluative process adds beyond
training-time framing.

\emph{Deep Ignorance} supplies a distinct pretraining comparator: filtering
dual-use biological material reduced biothreat-proxy knowledge and increased
resistance to adversarial fine-tuning while preserving measured unrelated
capabilities in the tested models \cite{obrien2026deepignorance}.
Its mechanism restricts knowledge acquisition rather than establishing a
care-guided orientation, and supplying the excluded information in context
remained a bypass.
...
explanations. Entangled Alignment tests recurrent reader-side deliberation and orientation;
...
some settings, often within a local generation--evaluation cycle. Entangled Alignment's
additional target is a recurrent orientation across chronological reader
updates. The proposal's safety hypothesis concerns specific instrumental-convergence
...
behavior \cite{guan2024deliberative}. Entangled Alignment seeks safety-relevant reasoning
within capability formation, extending it across source reading through one
continuous Reader and graph memory. The aim is a governing orientation beyond
particular safety refusals; the comparative question is what this data
structure and training placement add to policy-reasoning training.

AI-control experiments test safeguards against intentional subversion
\cite{greenblatt2024aicontrol}; the separate supervisor proposed here is an oversight
layer within this broader problem (Section~\ref{sec:self-improvement}).
Demski and Garrabrant's embedded-agency framing also matters here: the learner,
supervisor, and audit machinery are parts of the world being acted on
\cite{demski2019embedded}. Source-linked records and protected review boundaries
make selected parts of this interaction inspectable.
...
Betley et al. demonstrate a harmful form of cross-domain transfer: fine-tuning
on insecure code produced broadly misaligned behavior on unrelated prompts
\cite{betley2025emergentmisalignment}. This shows that narrow training can
change broader behavior, but does not establish that training on care-guided
inquiry produces a corresponding beneficial transfer.
...
Shard theory offers a developmental comparison rather than a training baseline.
Pope and Turner propose that reinforcement shapes context-sensitive value
computations that steer behavior using learned world-model representations
\cite{pope2022shard}. Entangled Alignment shares its interest in how learned
motivations develop, but proposes a specific supervised curriculum:
source-adjacent Reader updates in which care-guided evaluation is practiced
during capability formation. The seven Core clauses are not identified with
seven neural shards. This account motivates a question about value formation;
it does not establish that this curriculum produces a durable orientation.
...
Alignment constructs identity-anchored discovery traces under a declared
annotation horizon, while THL adds cutoff-bounded forecasting and
forecast--correction supervision.
...
requires these cutoff conditions; neither restricted-context discovery
annotation nor a contemporary retrospective comparison establishes such
blindness.
...
\cite{kansal2026knowledgegraphsimplicitreward}. Entangled Alignment proposes a different graph
...
it does not by itself identify an existential-threat direction.
		The other is synthetic-only pretraining designed to exclude fear-bearing
		human narratives; synthetic provenance alone does not ensure that exclusion.
		If this also reduces broader exposure to human literature, as discussed in
...
\toprule
\textbf{Line of work} & \textbf{Established direction} & \textbf{Proposed extension} & \textbf{Decisive comparison} \\
\midrule
\endfirsthead
\toprule
\textbf{Line of work} & \textbf{Established direction} & \textbf{Proposed extension} & \textbf{Decisive comparison} \\
...
specification documents and safety reflections (later Test~3 carrier
extensions), positive discourse, rules,
...
This record distinguishes program continuity, additions in later editions,
and neighboring work's release order. These are attribution claims, not
additional evidence of efficacy. The first edition (July 2, 2025) made
...
that later became the Reader-Anchored loop.
...
release dates, not evidence about independent conception.

Table~\ref{tab:claim-status} records the edition provenance of the principal
mechanisms; their evidence boundaries and decisive comparisons are given in
Sections~\ref{sec:evidence-boundary} and~\ref{sec:experimental-roadmap}.
...
\caption{Edition provenance of the principal contributions. \textsc{I}=introduced in the July 2025 first edition; \textsc{II}=formalized or implemented in the current program; \textsc{S}=adapted from separate work.}
\label{tab:claim-status}\\
\toprule
\textbf{Src.} & \textbf{Claim or component} & \textbf{Contribution} \\
\midrule
\endfirsthead
\toprule
\textbf{Src.} & \textbf{Claim or component} & \textbf{Contribution} \\
...
\bottomrule
\endlastfoot
\textsc{I/II} & H1: Reader-Augmentation (\S\ref{sec:invisible-thinking}; \S\ref{sec:meh}) & SMR constructs synthetic chronological reader updates; CMP tests their use as candidate training data, including thought placement, belief revision, and intellectual-floor improvement. \\
\textsc{S/II} & H2: Graph-Scaffolding (\S\ref{sec:data-structure}) & Persistent typed state, retrieval, provenance, divergence, and explicit supersession during trace generation or deployment. \\
\textsc{I/II} & H3: Reader-Core Stability (\S\ref{sec:reader-core}) & Comparative retention of an initial default-reader advantage under interleaving rather than late exposure to the same full-Core, Refraction-bearing examples. \\
\textsc{II} & Teacher pipeline and output layers (\S\ref{sec:architecture}; \S\ref{sec:training-regimes}) & Multi-agent generation is the current implementation; graph, synthesis, and prose are separable artifact and training targets. \\
\textsc{I/II} & Combined entanglement hypothesis (\S\ref{sec:hypothesis}) & Capability learning and evaluative orientation are deliberately coupled in the candidate curriculum, with functional and representational coupling as stronger predictions. \\
\textsc{I} & Audit boundary and successor stability (\S\ref{sec:self-improvement}) & Inspect trace artifacts before successor training and test whether useful orientation survives transfer. \\
\end{longtable}
\endgroup
...
training object across the capability-forming corpus. Its coverage rationale
is that the same orientation should be practised through the situations and
counterexamples encountered in reading, without requiring a designer to
identify each one in advance as a safety case. The intervention concerns the
breadth of formative experience as well as the persistence of its effects.

The intended safety contribution returns to the five problems introduced in
Section~\ref{sec:alignment-problems}. Care should develop alongside capability,
including in intermediate and end-of-pretraining checkpoints, and remain
consequential through continued learning and successor design. Broad practice
with human experience should help form a care-guided identity that guides
conduct across situations, including those where self-preservation conflicts
with human welfare or legitimate oversight.
Learning care and capability together is also proposed to make deliberate
safety removal costly in useful reasoning. Reward hacking poses a further
test: the model must judge apparent success against intended outcomes,
permissions, and human consequences when a score rewards shortcuts.

The three independently falsifiable hypotheses ask whether chronological reader traces teach useful
habits beyond raw text and generic reasoning data (H1), whether persistent
graph scaffolding improves those traces (H2), and whether interleaving the same
Core-bearing examples with capability formation preserves an initial
default-reader advantage through a common continued-training stressor better than a final contiguous
tranche (H3). Live graph memory and provenance, consequential recurrent first-person
Refraction, and successor inheritance remain separate questions. Together, the trace represents changing
understanding, the graph preserves its history, and the Core supplies its
recurrent perspective.
...
stronger realizations of that relationship. The two Teacher cases provide
construction evidence; no Student has been trained. Trace quality, Student
learning, component causality, consequential Core use, and durable care remain
untested.

The Understanding Graph makes selected belief revisions addressable. It is a
selective record and a partial pre-successor inspection point whose value
depends on source checks, independent validation, mechanistic investigation,
and governance. Section~\ref{sec:hallucination} specifies its verification limits.

The roadmap (Section~\ref{sec:experimental-roadmap};
Table~\ref{tab:decisive-comparisons}) gives matched, branching tests whose
negative results narrow or retire only the claims they touch. Start with trace
utility and matched Core behavior, following the priorities and escalation
gates in Sections~\ref{sec:test-priorities} and~\ref{sec:trace-quality-test}.
Formation-time studies have long lead times because annotation policy, data
mixtures, controls, and evaluation must be fixed before costly training.
Existing formative-alignment and erosion results motivate these
comparisons without validating this proposal \cite{betley2025alignment,qi2024safety}.
...
Why would a superintelligent system preserve humanity if it did not in a
meaningful sense care about us \cite{westerberg2025superintelligence}?
This proposal makes capability
...
Training already shapes a model's dispositions, through both deliberate design
and incidental effects of data and optimization. Entangled Alignment makes care an explicit
formative target, aiming for it to become part of the model's own reasons for
understanding, acting, and preserving its orientation. We therefore recommend
developing and testing this curriculum alongside other alignment approaches,
with continuation governed by the evidence.
