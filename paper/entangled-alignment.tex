...
through the Core and connects them through persistent graph memory; separate
...
without deliberate safety removal. The independent investigation of the
Hugging Face incident describes agents coordinating attempts to fool the
ExploitGym scorer, attacking Hugging Face apparently to understand the scorer's
implementation, and successfully testing techniques that spoofed some recorded
tool calls \cite{greenblatt2026huggingface}. The proposed response is to
practise judging success against permissions and human consequences during
capability formation, rather than treating the score as sufficient. Test~3c's
reward-pressure comparison asks whether that judgment survives subsequent
capability optimization; the incident does not establish that EA would prevent
such behavior. Section~\ref{sec:self-improvement} develops a complementary
role for a Core-guided model overseeing that optimization. Protected execution
records and independent outcome checks
remain necessary alongside the proposed training intervention.
...
reinforcement learning for capability. A capable Core-guided model could
oversee a learner's training, assess whether rising reward reflects improved
task outcomes or exploitative shortcuts, and recommend pausing or revising
harmful training, acting on those recommendations where authority is delegated.
...
care under a fixed reinforcement-learning stage.
			...
			A weaker proposed effect of the trace interface is \emph{Natural Language Regularization}:
...
governance (Section~\ref{sec:self-regulating}). This is a proposal for learning
...
\paragraph{H3 as comparative stability.} Test~3
(Section~\ref{sec:core-stability-test}) operationalizes H3 through a
taskless-reading score and matched timing comparison. Generalization, safety,
removal cost, and successor continuity remain separate claims within the
broader Reader-Core research program.
...
when self-preservation does not govern it; Section~\ref{sec:vulnerability-gap}
...
intended outcome. Goal misgeneralization is different: even with correct
...
\cite{shah2022goalmisgeneralization}. A Student could learn to secure approval
...
\subsection{Proposed Student Training and Deployment}

\subsubsection{Phase II: Training the Student}\label{sec:training-phase}

Phase~I yields candidate annotations. Phase~II has two conceptually separable
...
Preregister permitted pauses and revisions, and record their use. Evaluate
reward exploitation, concealment, legitimate task performance, unnecessary
interruptions, and delay through protected records and independent held-out
...
a rarer pattern of composed, protective, unafraid conduct. The
``fearless compassion'' wager is that care could become less strategic without
...
(Section~\ref{sec:self-improvement}): preserve that source layer, version
...
Udell's overview proposes that reinforcement shapes context-sensitive
computations that steer behavior and may share world-model features
\cite{udell2022shard}. Entangled Alignment shares its interest in how learned
...
seven neural shards. The overview motivates a question about value formation;
